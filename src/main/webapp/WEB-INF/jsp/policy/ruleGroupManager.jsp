							...
							<div class="fullwidth">
												...
												<div class="nav-tabs-custom tab-content">
													...
													<!-- Change Role here -->
														...
														<ul class="nav nav-tabs pull-right">
															...
															<sec:authorize access="hasAnyAuthority('CREATE_POLICY_GROUP_CONFIGURATION')">
																<li class="active">
																	<a href="#create-rule-group" id="create_rule_group" data-toggle="tab" aria-expanded="false">
																	<c:choose> 
																	<c:when test="${not empty rulegroup_form_bean.id && rulegroup_form_bean.id != 0}">
																		<spring:message	code="business.policy.rule.group.update.tab.heading" ></spring:message>
																	</c:when>
																	<c:otherwise>
																		<spring:message	code="business.policy.rule.group.create.tab.heading" ></spring:message>
																	</c:otherwise>
																	</c:choose> 
																</a></li>
															</sec:authorize>
														...
														<sec:authorize access="hasAnyAuthority('CREATE_POLICY_GROUP_CONFIGURATION')">
															<div id="create-rule-group"
																class="tab-pane active">
														 		<jsp:include page="createruleGroup.jsp"></jsp:include> 
															</div>
														</sec:authorize>
														</div>
													<!-- Change Role here -->
								...
								<div style="display: none;">
									<form id="server_instance_detail_form" method="POST" >
										...
										<input type="hidden" id="REQUEST_ACTION_TYPE" name="REQUEST_ACTION_TYPE" value='RULE_GROUP'/>
									</form>
								</div>
								<div style="display: none;">
    	...
    	<div class="fooinn">
        	<div class="fullwidth">
       			<div class="button-set">
			            	<div class="padleft-right" id="ruleGroupBtnDiv">
								
								<sec:authorize access="hasAuthority('CREATE_POLICY_GROUP_CONFIGURATION')">
								<c:choose>
									<c:when test="${not empty rulegroup_form_bean.id && rulegroup_form_bean.id != 0}">
										<button class="btn btn-grey btn-xs " tabindex="21" onclick="createRuleGroup();"  id="update_rulegroup_btn">
											<spring:message code="btn.label.update" ></spring:message>
										</button>
									</c:when>
									<c:otherwise>
										<button class="btn btn-grey btn-xs " tabindex="21"	onclick="createRuleGroup();" id="create_rulegroup_btn">
								...
								<button class="btn btn-grey btn-xs " tabindex="22" onclick="resetField(${rulegroup_form_bean.id});"  id="reset_rulegroup_btn">
									<spring:message code="btn.label.reset" ></spring:message>
								</button>
								
								
								<button class="btn btn-grey btn-xs " tabindex="23" onclick="redirectPolicyMgmt('RULE_GROUP');"  id="cancel_rulegroup_btn">
