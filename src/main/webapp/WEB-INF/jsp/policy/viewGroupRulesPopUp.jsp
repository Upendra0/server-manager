...
<script>
var id;
function loadJQueryGroupRuleGrid1($ruleGroupId){
	id = $ruleGroupId;
}
...
function loadJQueryGroupRuleGrid($ruleGroupId){
	clearGroupRuleGrid();
	$("#groupRuleGrid").jqGrid({
		...
		colModel: [
			...
			{name: 'applicationOrder', index: 'applicationOrder', sortable: true, align:'center'},
			{name:'name',index:'policyRule.name', sortable: true, align:'center'},
			{name:'description',index:'policyRule.description', align:'center'}
		],
		...
		pgtext : "<spring:message code="collectionService.summary.agent.grid.pager.text"></spring:message>",
			...
			//$("#groupRuleGrid").sortableRows();   
			$("#groupRuleGrid").disableSelection();
			
			
			$("#groupRuleGrid").jqGrid('setGridParam', { 
		        postData: {'ruleGroupId': $ruleGroupId }
		 	}).trigger("reloadGrid", [{current: true}]);
			
	//		$("#groupRuleGrid").trigger("reloadGrid", [{current: true}]);
}
 ...
 <div class="modal-content">
		        <div class="modal-header padding10">
		            <h4 class="modal-title"><spring:message code="business.policy.rule.group.grid.heading"></spring:message></h4>
		        </div>
					
	<div class="box-body table-responsive no-padding box">
		<table class="table table-hover" id="groupRuleGrid"></table>
		<div id="groupRuleGridPagingDiv"></div>
	    ...
	    <div id="divLoading" align="center" style="display: none;"><img src="img/preloaders/Preloader_10.gif" /></div>
		        ...
		        <div id="delete-buttons-div" class="modal-footer padding10">
		            <button type="button" class="btn btn-grey btn-xs " id="btnGroupRuleListClose" data-dismiss="modal" onclick="closeFancyBox();clearSelection();"><spring:message code="btn.label.close"></spring:message></button>
