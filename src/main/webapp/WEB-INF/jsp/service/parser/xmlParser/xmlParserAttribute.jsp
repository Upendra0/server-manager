...
<div class="tab-content no-padding  mtop15">
	<div class="fullwidth mtop15" id="attribute_details_div">
		<form:form modelAttribute="<%=FormBeanConstants.XML_PARSER_MAPPING_FORM_BEAN %>" method="POST"
			...
			<div class="modal-content">
				...
				<sec:authorize access="hasAuthority('DELETE_PARSER')">
					<div id="delete_attribute_bts_div" class="modal-footer padding10">
						<button id="xml_del_btn" type="button" class="btn btn-grey btn-xs"
...
function addBasicDetail(){
    ...
    $.ajax({
		url: '<%=ControllerConstants.UPDATE_XML_PARSER_MAPPING%>',
		data : oMyForm ,
        type : "POST",
        datatype: "json",
        processData: false, 
...
function createXMLAttributeGrid(defaultRowNum){				
	 ...
	 $("#xmlAttributeList").jqGrid({	    	
		...
		$("#xmlAttributeList").sortableRows();   
		$("#xmlAttributeList").disableSelection();
	    $("#xmlAttributeList").sortable({
	    	items: 'tr:not(:first)'
	    });
	    
	    $('#xmlAttributeList').navButtonAdd('#xmlAttributeListDiv', {
           title: "Update Order",
...
function updateOrder(){
	...
	var ids = $('#xmlAttributeList').jqGrid('getDataIDs');
	window.scrollTo(0, 0);
	resetWarningDisplay();
	clearAllMessages();
...
function reloadXMLAttributeGridData(){
	clearAllMessages();
	
	var $grid = $("#xmlAttributeList");
