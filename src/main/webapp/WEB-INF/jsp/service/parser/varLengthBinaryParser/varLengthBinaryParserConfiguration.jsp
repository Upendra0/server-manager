	...
	<div class="fullwidth mtop15">
		...
		<!-- Device details Section end here    -->
			...
			<div class="fullwidth" id="advance_details_div">
			 	<jsp:include page="varLengthBinaryAdvanceDetail.jsp"></jsp:include>
			</div> 
			<!-- parser Advance details end here -->
			
			<div class="fullwidth" id="data_detfinition_div">
				<jsp:include page="../varLengthAsciiParser/varLengthAsciiDataDefinition.jsp"></jsp:include>
			</div>
...
<!-- Customize mapping pop-up code start here  -->
   ...
   <form:form modelAttribute="var_length_binary_parser_mapping_form_bean" method="POST" action="<%= ControllerConstants.CREATE_UPDATE_MAPPING_DETAILS %>" id="create-mapping-form">
...
function setVarLengthBinaryParserAdvanceDetail(responseObject){
	
	if(responseObject.skipFileHeader === false){
		$("#skipFileHeader").val('false');
	}else if(responseObject.skipFileHeader === true){
		$("#skipFileHeader").val('true');
	}
	if(responseObject.skipSubFileHeader === false){
		$("#skipSubFileHeader").val('false');
	}else if(responseObject.skipSubFileHeader === true){
		$("#skipSubFileHeader").val('true');
	}
	$("#fileHeaderSize").val(responseObject.fileHeaderSize);
	$("#subFileHeaderSize").val(responseObject.subFileHeaderSize);
   	$("#subFileLength").val(responseObject.subFileLength);
   	$("#extractionRuleKey").val(responseObject.extractionRuleKey);
   	$("#extractionRuleValue").val(responseObject.extractionRuleValue);
   	$("#recordLengthAttribute").val(responseObject.recordLengthAttribute);
