...
<div class="wrapper"> 
    ...
    <div class="content-wrapper"> 
        ...
        <section class="content-header">
                                        ...
                                        <div class="box-body table-responsive no-padding">
                                            <div class="nav-tabs-custom"> 
                                                ...
                                                <ul class="nav nav-tabs pull-right">
                                                    ...
                                                    <li class="<c:if test="${(REQUEST_ACTION_TYPE eq 'VAR_LENGTH_ASCII_PARSER_CONFIGURATION')}"><c:out value="active"></c:out></c:if>" onclick="showButtons('parser_configuration'); getVarLengthAsciiParserConfig('<%=ControllerConstants.INIT_VAR_LENGTH_ASCII_PARSER_CONFIG%>');" >
                                                    	<a href="#varLengthAsciiConfiguration" data-toggle="tab" id="parser_configuration"><spring:message code="parsing.service.config.tab.header"></spring:message></a>
                                                    </li>
                                                    <li class="<c:if test="${(REQUEST_ACTION_TYPE eq 'VAR_LENGTH_ASCII_PARSER_ATTRIBUTE')}"><c:out value="active" ></c:out></c:if>" onclick="showButtons('parser_attribute_configuration'); getVarLengthAsciiParserAttrDetails('<%=ControllerConstants.INIT_VAR_LENGTH_ASCII_PARSER_ATTRIBUTE%>');" >
                                                    	<a href="#varLengthAsciiAttribute" data-toggle="tab" id="parser_attribute_configuration"><spring:message code="parsing.service.attr.list.tab.header"></spring:message></a>
                                                    </li>
                                                  </sec:authorize>  
                                                </ul>
                                                <div class="fullwidth tab-content no-padding">
                                                <sec:authorize access="hasAnyAuthority('VIEW_PARSER')">
                                                	<div class="tab-pane <c:if test="${(REQUEST_ACTION_TYPE eq 'VAR_LENGTH_ASCII_PARSER_CONFIGURATION')}"><c:out value="active"></c:out></c:if>" id="varLengthAsciiConfiguration">
                                                		 <c:if test="${(REQUEST_ACTION_TYPE eq 'VAR_LENGTH_ASCII_PARSER_CONFIGURATION')}">
                                                			<jsp:include page="varLengthAsciiParserConfiguration.jsp" ></jsp:include>
                                                		</c:if>
                                                	</div>
                                                	
                                                	<div class="tab-pane <c:if test="${(REQUEST_ACTION_TYPE eq 'VAR_LENGTH_ASCII_PARSER_ATTRIBUTE')}"><c:out value="active"></c:out></c:if>" id="varLengthAsciiAttribute">
                                               			<c:if test="${(REQUEST_ACTION_TYPE eq 'VAR_LENGTH_ASCII_PARSER_ATTRIBUTE')}">
                                               				<jsp:include page="varLengthAsciiParserAttribute.jsp" ></jsp:include>
                                               			</c:if>
                                                	</div>
                                                	</sec:authorize>
        	...
        	<div style="display: block;">
				...
				<form id="var-length-ascii-parser-form" action = "" method="">
	...
	<footer class="main-footer positfix">
    	<div class="fooinn">
        	<div class="fullwidth">
       			<div class="padleft-right" id="varLengthAsciiParserConfigDiv" style="display: none;">
            	...
            	<div class="padleft-right" id="var_length_ascii_attribute_default" style="display: none;">
				</div>
        	</div>
        	<script>
				function showHideButtonBasedOnTabsSelected(tabType){
					$("#varLengthAsciiParserConfigDiv").hide();
					$("#var_length_ascii_attribute_default").hide();
					
					if(tabType == 'VAR_LENGTH_ASCII_PARSER_CONFIGURATION'){
						$("#varLengthAsciiParserConfigDiv").show();
					}else if(tabType == 'VAR_LENGTH_ASCII_PARSER_ATTR_LIST'){
						$("#var_length_ascii_attribute_default").show();
					}
				}
				
				function showButtons(id){
					if(id == 'parser_configuration'){
						showHideButtonBasedOnTabsSelected('VAR_LENGTH_ASCII_PARSER_CONFIGURATION');
					}else if(id == 'parser_attribute_configuration'){
						showHideButtonBasedOnTabsSelected('VAR_LENGTH_ASCII_PARSER_ATTR_LIST');
					}
				}
			...
			$(document).ready(function() {
				...
				if(readOnlyFlag == '' || readOnlyFlag ==true){
					disableVarLengthAsciiAdvanceDetails();	
					disableVarLengthAsciiDataDefinition();
				}
				...
				if(parserDeviceTypeId > 0 && currentTab == 'VAR_LENGTH_ASCII_PARSER_CONFIGURATION'){
			...
			function getVarLengthAsciiParserConfig(action){
			
				$("#var-length-ascii-parser-form").attr("action",action);
				$("#var-length-ascii-parser-form").attr("method","GET");
				
				$("#var-length-ascii-parser-form #deviceName").val($("#deviceName").find(":selected").text());
				$("#var-length-ascii-parser-form #mappingName").val($("#mappingName").find(":selected").text());
				$("#var-length-ascii-parser-form #mappingId").val($("#mappingName").find(":selected").val());
				$("#var-length-ascii-parser-form #deviceId").val($("#deviceName").find(":selected").val());
				
				$("#var-length-ascii-parser-form").submit();
			}
			
			function getVarLengthAsciiParserAttrDetails(action){
				
				$("#var-length-ascii-parser-form").attr("action",action);
				$("#var-length-ascii-parser-form").attr("method","POST");
				
				$("#var-length-ascii-parser-form #deviceName").val($("#deviceName").find(":selected").text());
				if($("#var-length-ascii-parser-form #deviceName").val() == ''){
					$("#var-length-ascii-parser-form #deviceName").val('${deviceName}')
				}
				
				$("#var-length-ascii-parser-form #mappingName").val($("#mappingName").find(":selected").text());
				if($("#var-length-ascii-parser-form #mappingName").val() == ''){
					$("#var-length-ascii-parser-form #mappingName").val('${mappingName}');
				}
				
				$("#var-length-ascii-parser-form #mappingId").val($("#mappingName").find(":selected").val());
				if($("#var-length-ascii-parser-form #mappingId").val() == ''){
					$("#var-length-ascii-parser-form #mappingId").val("${mappingId}");
				}
				$("#var-length-ascii-parser-form #deviceId").val($("#deviceName").find(":selected").val());
				if($("#var-length-ascii-parser-form #deviceId").val() == ''){
					$("#var-length-ascii-parser-form #deviceId").val('${deviceId}');
				}
				$("#var-length-ascii-parser-form").submit();
			}
			...
			function setReadOnlyFlag(){
				...
				var fileType=$("#fileTypeEnum").val();
				if(fileType == '<%=BaseConstants.KEY_VALUE_RECORD %>'){
				...
				}else if(fileType == '<%=BaseConstants.RECORD_HEADER %>'){
					...
					if(recordSep != 'Other'){
						...
						$("#recordHeaderSeparator").val(recordSep);
					}
					var fieldSep = $('#fieldSepValues').val();
					if(fieldSep != 'Other'){
						if(fieldSep == 's'){
				...
				}else if(fileType == '<%=BaseConstants.FILE_HEADER_FOOTER %>'){
					var fieldSep = $('#fieldSepValues').val();
					if(fieldSep != 'Other'){
						if(fieldSep == 's'){
							$("#fieldSeparator").val(' ');
				...
				}else if(fileType == '<%=BaseConstants.DELIMITER%>'){
					var fieldSep = $('#fieldSepValues').val();
					if(fieldSep != 'Other'){
						if(fieldSep == 's'){
							$("#fieldSeparator").val(' ');
				...
				}else if(fileType == '<%=BaseConstants.LINEAR_KEY_VALUE_RECORD%>'){
					var keyValSep = $('#keyValueSepValues').val();
					if(keyValSep != 'Other'){
						if(keyValSep == 's'){
							$("#keyValueSeparator").val(' ');
			...
			function disableVarLengthAsciiAdvanceDetails(){
			...
			function enableVarLengthAsciiAdvanceDetails(){
				$("#advance_details_div_mask").remove();
			...
			/* Function will disable the content */
			function disableVarLengthAsciiDataDefinition(){
			...
			/*Function will enable the mapping details*/
			function enableVarLengthAsciiDataDefinition(){
				$("#data_detfinition_div_mask").remove();
