...
<div class="tab-content no-padding  mtop15">
	<div class="fullwidth mtop15" id="attribute_details_div">
		<form:form modelAttribute="<%=FormBeanConstants.MTSIEMENS_PARSER_MAPPING_FORM_BEAN %>" method="POST"
			id="mtsiemens_attribute_configuration_form_bean">
		...
		<div class="fullwidth" id="add_delete_attribute_link_div">
			<div class="title2">
				...
				<span class="title2rightfield-icon1-text">
					<a onclick="addColNames('#mtsiemensAttributeList');downloadCSVFile();">
						<i class="fa fa-download"></i>
					</a> 
					<a href="#" onclick="addColNames('#mtsiemensAttributeList');downloadCSVFile();">
						<spring:message code="parserConfiguration.attribute.grid.exportattribute.label" ></spring:message>
					</a>
				</span> 
				<sec:authorize access='hasAuthority(\'CREATE_PARSER\')'>
				<span class="title2rightfield-icon1-text"> 
					<a href="#" onclick="openUploadAttrPopup('#mtsiemensAttributeList');">
		...
		<div class="col-md-12 inline-form no-padding" id="attribute_grid_div">
			<div class="box-body table-responsive no-padding box"
				id="parser_attribute">
				<table class="table table-hover" id="mtsiemensAttributeList"></table>
				<div id="mtsiemensAttributeListDiv"></div>
			...
			<div class="modal-content">
				...
				<sec:authorize access="hasAuthority('DELETE_PARSER')">
					<div id="delete_attribute_bts_div" class="modal-footer padding10">
						<button id="mtsiemens_del_btn" type="button" class="btn btn-grey btn-xs"
...
<script type="text/javascript">
	...
	createMtsiemensAttributeGrid('<%=MapCache.getConfigValueAsInteger(SystemParametersConstant.TOTAL_ROWS_TO_DISPLAY_IN_GRID, 10)%>');
...
function addBasicDetail(){
    ...
    $.ajax({
		url: '<%=ControllerConstants.UPDATE_MTSIEMENS_PARSER_MAPPING%>',
		data : oMyForm ,
        type : "POST",
        datatype: "json",
        processData: false, 
...
function getAllMtsiemensAttributeListByMappingId(urlAction, mappingId){
	...
	$.ajax({
		...
		success: function(data){						
			...
			var responseObject = response.object;			
			if(responseCode == "200"){				
				$("#mtsiemensAttributeListDiv").show();				
				...
				if(attributeList!=null && attributeList != 'undefined' && attributeList != undefined){					
					...
					jQuery("#mtsiemensAttributeList").jqGrid('setGridParam', {datatype: 'local', data:gridArray }).trigger("reloadGrid");
				} 
...
function createMtsiemensAttributeGrid(defaultRowNum){	
	 $("#mtsiemensAttributeList").jqGrid({	    	
	        datatype: "local",
	        colNames:[
	                  "#",
			...
			colModel:[
			          ...
			          {name: 'checkbox', index: 'checkbox',sortable:false, formatter:mtsiemensParserAttributeCheckboxFormatter, align : 'center', width:'30%', search : false},
	        ...
	        height: 'auto',
	        pager: '#mtsiemensAttributeListDiv',
	        viewrecords: true,
	        search:true,
	        ignoreCase: true,
	        timeout : 120000,
	 	...
	 	}).navGrid("#mtsiemensAttributeListDiv",{edit:false,add:false,del:false,search:true});
		$(".ui-jqgrid-titlebar").hide();
		$('#mtsiemensAttributeList').jqGrid('filterToolbar',{
			stringResult: true,
		...
		$("#mtsiemensAttributeList").sortableRows();   
		$("#mtsiemensAttributeList").disableSelection();
	    $("#mtsiemensAttributeList").sortable({
	    	items: 'tr:not(:first)'
	    });
	    
	    $('#mtsiemensAttributeList').navButtonAdd('#mtsiemensAttributeListDiv', {
           title: "Update Order",
 ...
 */
function mtsiemensParserAttributeCheckboxFormatter(cellvalue, options, rowObject) {
	var uniqueId = getUniqueCheckboxIdForAttribute(rowObject);
	return '<input type="checkbox" id="'+uniqueId+'" name="attributeCheckbox" value="'+rowObject["id"]+'" onchange="updateRootCheckbox(this, \'mtsiemensAttributeList\')"/>';
}

function updateOrder(){
	console.log("updateOrder Called");
	var ids = $('#mtsiemensAttributeList').jqGrid('getDataIDs');
	window.scrollTo(0, 0);
	resetWarningDisplay();
	clearAllMessages();
...
function reloadMtsiemensAttributeGridData(){
	clearAllMessages();
	
	var $grid = $("#mtsiemensAttributeList");
