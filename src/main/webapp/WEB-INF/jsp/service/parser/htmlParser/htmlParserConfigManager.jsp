...
var jsSpringMsg = {};
	...
	jsSpringMsg.fieldidentifier="<spring:message code='parsing.service.attr.grid.column.source.field.identifier' ></spring:message>";
	jsSpringMsg.tdNo="<spring:message code='parsing.service.attr.grid.column.td.no' ></spring:message>";
	jsSpringMsg.fieldextractionmethod="<spring:message code='parsing.service.attr.grid.column.source.field.extraction.method' ></spring:message>";
    jsSpringMsg.containsfieldattribute="<spring:message code='parsing.service.attr.grid.column.source.field.attribute' ></spring:message>";
    jsSpringMsg.valueseparatorattribute="<spring:message code='parsing.service.attr.grid.column.valueSeparator.field.attribute' ></spring:message>";
    jsSpringMsg.valueindexattribute="<spring:message code='parsing.service.attr.grid.column.valueIndex.attribute' ></spring:message>";
    jsSpringMsg.fieldSectionId="<spring:message code='parsing.service.attr.grid.column.field.section.id' ></spring:message>";
	...
	jsSpringMsg.htmlParser='<%=EngineConstants.HTML_PARSING_PLUGIN%>';
	...
	<div class="wrapper">
		...
		<div class="content-wrapper">
												...
												<div class="nav-tabs-custom">
													...
													<ul class="nav nav-tabs pull-right">
														<sec:authorize access="hasAnyAuthority('VIEW_PARSER')">
															<li
																class="<c:if test="${(REQUEST_ACTION_TYPE eq 'HTML_PARSER_CONFIGURATION')}"><c:out value="active"></c:out></c:if>"
																onclick="showButtons('parser_configuration'); gethtmlParserConfig('<%=ControllerConstants.INIT_HTML_PARSER_CONFIG%>');">
																<a href="#htmlConfiguration" data-toggle="tab"
																id="parser_configuration"><spring:message
																		code="parsing.service.config.tab.header" ></spring:message></a>
															</li>
															<li
																class="<c:if test="${(REQUEST_ACTION_TYPE eq 'HTML_PARSER_ATTRIBUTE')}"><c:out value="active" ></c:out></c:if>"
																onclick="showButtons('parser_attribute_configuration'); gethtmlParserAttrDetails('<%=ControllerConstants.INIT_HTML_PARSER_ATTRIBUTE%>');">
																<a href="#htmlAttribute" data-toggle="tab"
																id="parser_attribute_configuration"><spring:message
																		code="parsing.service.attr.list.tab.header" ></spring:message></a>
															</li>
															<li
																class="<c:if test="${(REQUEST_ACTION_TYPE eq 'HTML_PARSER_GROUP_ATTRIBUTE')}"><c:out value="active" ></c:out></c:if>"
																onclick="showButtons('parser_attribute_configuration'); gethtmlParserGroupAttrDetails('<%=ControllerConstants.INIT_HTML_PARSER_GROUP_ATTRIBUTE%>','true');">
																<a href="#htmlGroupAttribute" data-toggle="tab"
													...
													<div class="fullwidth tab-content no-padding">
														<sec:authorize access="hasAnyAuthority('VIEW_PARSER')">
															<div
																class="tab-pane <c:if test="${(REQUEST_ACTION_TYPE eq 'HTML_PARSER_CONFIGURATION')}"><c:out value="active"></c:out></c:if>"
																id="htmlConfiguration">
																<c:if
																	test="${(REQUEST_ACTION_TYPE eq 'HTML_PARSER_CONFIGURATION')}">
																	<jsp:include page="htmlParserConfiguration.jsp" ></jsp:include>
																</c:if>
															</div>

															<div
																class="tab-pane <c:if test="${(REQUEST_ACTION_TYPE eq 'HTML_PARSER_ATTRIBUTE')}"><c:out value="active"></c:out></c:if>"
																id="htmlAttribute">
																<c:if
																	test="${(REQUEST_ACTION_TYPE eq 'HTML_PARSER_ATTRIBUTE')}">
																	<jsp:include page="htmlParserAttribute.jsp" ></jsp:include>
																</c:if>
															</div>

															<div
																class="tab-pane <c:if test="${(REQUEST_ACTION_TYPE eq 'HTML_PARSER_GROUP_ATTRIBUTE')}"><c:out value="active"></c:out></c:if>"
																id="htmlGroupAttribute">
																<c:if
																	test="${(REQUEST_ACTION_TYPE eq 'HTML_PARSER_GROUP_ATTRIBUTE')}">
																	<jsp:include page="htmlParserGroupAttribute.jsp" ></jsp:include>
																</c:if>
															</div>
														</sec:authorize>
			...
			<div style="display: block;">
				...
				<form id="html-parser-form" action="" method="">
				...
				<form id="go-to-service-form"
					action="<%=ControllerConstants.INIT_UPDATE_SERVICE %>" method="GET">
					<input type="hidden" id="serviceId" name="serviceId" value="${serviceId}"/> 
					<input type="hidden" id="serviceType" name="serviceType" value="${serviceType}" />
					<input type="hidden" id="serviceName" name="serviceName" value="${serviceName}"/> 
					<input type="hidden" id="serviceInstanceId" name="serviceInstanceId" value="${serviceInstanceId}"> 
		...
		<footer class="main-footer positfix">
			<div class="fooinn">
				<div class="fullwidth">
					<div class="padleft-right" id="htmlParserConfigDiv"
						style="display: none;">
					...
					<div class="padleft-right" id="htmlParserConfigbtnDiv"
						style="display: none;">
						<c:if
							test="${(REQUEST_ACTION_TYPE eq 'HTML_PARSER_ATTRIBUTE') && (readOnlyFlag eq 'false') }">
							<button class="btn btn-grey btn-xs " id="move_to_next_attribute"
								type="button" onclick="gethtmlParserGroupAttrDetails('<%=ControllerConstants.INIT_HTML_PARSER_GROUP_ATTRIBUTE%>','false');">
								<spring:message code="btn.label.save.next" ></spring:message>
							</button>
						</c:if>
					</div>
					<div class="padleft-right" id="html_attribute_default"
						style="display: none;"></div>
				</div>
				<script>
				function showHideButtonBasedOnTabsSelected(tabType){
					$("#htmlParserConfigDiv").hide();
					$("#html_attribute_default").hide();
					
					if(tabType == 'HTML_PARSER_CONFIGURATION'){
						$("#htmlParserConfigDiv").show();
					}else if(tabType == 'HTML_PARSER_ATTR_LIST'){
						$("#html_attribute_default").show();
						$("#htmlParserConfigbtnDiv").show();
					}
				}
				
				function showButtons(id){
					if(id == 'parser_configuration'){
						showHideButtonBasedOnTabsSelected('HTML_PARSER_CONFIGURATION');
					}else if(id == 'parser_attribute_configuration'){
						showHideButtonBasedOnTabsSelected('HTML_PARSER_ATTR_LIST');
					}
				}
				...
				<script
					src="${pageContext.request.contextPath}/customJS/deviceManager.js<%= "?v=" + Math.random() %>"
			...
			$(document).ready(function() {
				...
				if(parserDeviceTypeId > 0 && currentTab == 'HTML_PARSER_CONFIGURATION'){
			...
			function gethtmlParserConfig(action){
				$("#html-parser-form").attr("action",action);
				$("#html-parser-form").attr("method","GET");				
				$("#html-parser-form #deviceName").val($("#deviceName").find(":selected").text());
				$("#html-parser-form #mappingName").val($("#mappingName").find(":selected").text());
				$("#html-parser-form #mappingId").val($("#mappingName").find(":selected").val());
				$("#html-parser-form #deviceId").val($("#deviceName").find(":selected").val());
				$("#html-parser-form").submit();
			}
			
			function gethtmlParserAttrDetails(action){
				
				$("#html-parser-form").attr("action",action);
				$("#html-parser-form").attr("method","POST");
				
				$("#html-parser-form #deviceName").val($("#deviceName").find(":selected").text());
				if($("#html-parser-form #deviceName").val() == ''){
					$("#html-parser-form #deviceName").val('${deviceName}')
				}
				
				$("#html-parser-form #mappingName").val($("#mappingName").find(":selected").text());
				if($("#html-parser-form #mappingName").val() == ''){
					$("#html-parser-form #mappingName").val('${mappingName}');
				}
				
				$("#html-parser-form #mappingId").val($("#mappingName").find(":selected").val());
				if($("#html-parser-form #mappingId").val() == ''){
					$("#html-parser-form #mappingId").val("${mappingId}");
				}
				$("#html-parser-form #deviceId").val($("#deviceName").find(":selected").val());
				if($("#html-parser-form #deviceId").val() == ''){
					$("#html-parser-form #deviceId").val('${deviceId}');
				}
				$("#html-parser-form").submit();
			}
			function gethtmlParserGroupAttrDetails(action,flag){
				$("#html-parser-form").attr("action",action);
				$("#html-parser-form").attr("method","POST");
				$("#html-parser-form #mappingId").val($("#mappingName").find(":selected").val());
				if($("#html-parser-form #mappingId").val() == ''){
				...
				$("#html-parser-form #readOnlyFlagGroup").val(flag);
				$("#html-parser-form").submit();
			}
			...
			function disableHtmlAdvanceDetails(){
			...
			function enablehtmlAdvanceDetails(){
				$("#advance_details_div_mask").remove();
