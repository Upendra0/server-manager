...
<div class="wrapper"> 
    ...
    <div class="content-wrapper"> 
        ...
        <section class="content-header">
                                        ...
                                        <div class="box-body table-responsive no-padding">
                                            <div class="nav-tabs-custom"> 
                                                ...
                                                <ul class="nav nav-tabs pull-right">
                                                    ...
                                                    <li class="<c:if test="${(REQUEST_ACTION_TYPE eq 'FIXED_LENGTH_ASCII_PARSER_CONFIGURATION')}"><c:out value="active"></c:out></c:if>" onclick="showButtons('parser_configuration'); getfixedLengthASCIIParserConfig('<%=ControllerConstants.INIT_FIXED_LENGTH_ASCII_PARSER_CONFIG%>');" >
                                                    	<a href="#fixedLengthASCIIConfiguration" data-toggle="tab" id="parser_configuration"><spring:message code="parsing.service.config.tab.header"></spring:message></a>
                                                    </li>
                                                    <li class="<c:if test="${(REQUEST_ACTION_TYPE eq 'FIXED_LENGTH_ASCII_PARSER_ATTRIBUTE')}"><c:out value="active" ></c:out></c:if>" onclick="showButtons('parser_attribute_configuration'); getfixedLengthASCIIParserAttrDetails('<%=ControllerConstants.INIT_FIXED_LENGTH_ASCII_PARSER_ATTRIBUTE%>');" >
                                                    	<a href="#fixedLengthASCIIAttribute" data-toggle="tab" id="parser_attribute_configuration"><spring:message code="parsing.service.attr.list.tab.header"></spring:message></a>
                                                    </li>
                                                  </sec:authorize>  
                                                </ul>
                                                <div class="fullwidth tab-content no-padding">
                                                <sec:authorize access="hasAnyAuthority('VIEW_PARSER')">
                                                	<div class="tab-pane <c:if test="${(REQUEST_ACTION_TYPE eq 'FIXED_LENGTH_ASCII_PARSER_CONFIGURATION')}"><c:out value="active"></c:out></c:if>" id="fixedLengthASCIIConfiguration">
                                                		 <c:if test="${(REQUEST_ACTION_TYPE eq 'FIXED_LENGTH_ASCII_PARSER_CONFIGURATION')}">
                                                			<jsp:include page="fixedLengthASCIIParserConfiguration.jsp" ></jsp:include>
                                                		</c:if>
                                                	</div>
                                                	
                                                	<div class="tab-pane <c:if test="${(REQUEST_ACTION_TYPE eq 'FIXED_LENGTH_ASCII_PARSER_ATTRIBUTE')}"><c:out value="active"></c:out></c:if>" id="fixedLengthASCIIAttribute">
                                               			<c:if test="${(REQUEST_ACTION_TYPE eq 'FIXED_LENGTH_ASCII_PARSER_ATTRIBUTE')}">
                                               				<jsp:include page="fixedLengthASCIIParserAttribute.jsp" ></jsp:include>
                                               			</c:if>
                                                	</div>
                                                	</sec:authorize>
        	...
        	<div style="display: block;">
				...
				<form id="fixedLengthASCII-parser-form" action = "" method="">
	...
	<footer class="main-footer positfix">
    	<div class="fooinn">
        	<div class="fullwidth">
       			<div class="padleft-right" id="fixedLengthASCIIParserConfigDiv" style="display: none;">
            	...
            	<div class="padleft-right" id="fixedLengthASCII_attribute_default" style="display: none;">
				</div>
        	</div>
...
<script>
	function showHideButtonBasedOnTabsSelected(tabType){
		$("#fixedLengthASCIIParserConfigDiv").hide();
		$("#fixedLengthASCII_attribute_default").hide();
		
		if(tabType == 'FIXED_LENGTH_ASCII_PARSER_CONFIGURATION'){
			$("#fixedLengthASCIIParserConfigDiv").show();
		}else if(tabType == 'FIXED_LENGTH_ASCII_PARSER_ATTRIBUTE'){
			$("#fixedLengthASCII_attribute_default").show();
		}
	}
	
	function showButtons(id){
		if(id == 'parser_configuration'){
			showHideButtonBasedOnTabsSelected('FIXED_LENGTH_ASCII_PARSER_CONFIGURATION');
		}else if(id == 'parser_attribute_configuration'){
			showHideButtonBasedOnTabsSelected('FIXED_LENGTH_ASCII_PARSER_ATTR_LIST');
		}
	}
...
$(document).ready(function() {
	...
	if(readOnlyFlag == '' || readOnlyFlag ==true){
		disablefixedLengthASCIIAdvanceDetails();	
	}
	...
	if(parserDeviceTypeId > 0 && currentTab == 'FIXED_LENGTH_ASCII_PARSER_CONFIGURATION'){
...
function getfixedLengthASCIIParserConfig(action){
	$("#fixedLengthASCII-parser-form").attr("action",action);
	$("#fixedLengthASCII-parser-form").attr("method","GET");
	
	$("#fixedLengthASCII-parser-form #deviceName").val($("#deviceName").find(":selected").text());
	$("#fixedLengthASCII-parser-form #mappingName").val($("#mappingName").find(":selected").text());
	$("#fixedLengthASCII-parser-form #mappingId").val($("#mappingName").find(":selected").val());
	$("#fixedLengthASCII-parser-form #deviceId").val($("#deviceName").find(":selected").val());
	
	$("#fixedLengthASCII-parser-form").submit();
}

function getfixedLengthASCIIParserAttrDetails(action){
	$("#fixedLengthASCII-parser-form").attr("action",action);
	$("#fixedLengthASCII-parser-form").attr("method","POST");
	
	$("#fixedLengthASCII-parser-form #deviceName").val($("#deviceName").find(":selected").text());
	if($("#fixedLengthASCII-parser-form #deviceName").val() == ''){
		$("#fixedLengthASCII-parser-form #deviceName").val('${deviceName}')
	}
	
	$("#fixedLengthASCII-parser-form #mappingName").val($("#mappingName").find(":selected").text());
	if($("#fixedLengthASCII-parser-form #mappingName").val() == ''){
		$("#fixedLengthASCII-parser-form #mappingName").val('${mappingName}');
	}
	
	$("#fixedLengthASCII-parser-form #mappingId").val($("#mappingName").find(":selected").val());
	if($("#fixedLengthASCII-parser-form #mappingId").val() == ''){
		$("#fixedLengthASCII-parser-form #mappingId").val("${mappingId}");
	}
	$("#fixedLengthASCII-parser-form #deviceId").val($("#deviceName").find(":selected").val());
	if($("#fixedLengthASCII-parser-form #deviceId").val() == ''){
		$("#fixedLengthASCII-parser-form #deviceId").val('${deviceId}');
	}
	$("#fixedLengthASCII-parser-form").submit();
}
...
function disablefixedLengthASCIIAdvanceDetails(){
...
function enablefixedLengthASCIIAdvanceDetails(){
	$("#advance_details_div_mask").remove();
