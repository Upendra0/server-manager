...
var jsSpringMsg = {};
	...
	jsSpringMsg.detailLocalParser='<%=EngineConstants.DETAIL_LOCAL_PARSING_PLUGIN%>'; 
...
<div class="wrapper"> 
    ...
    <div class="content-wrapper"> 
        ...
        <section class="content-header">
                                        ...
                                        <div class="box-body table-responsive no-padding">
                                            <div class="nav-tabs-custom"> 
                                                ...
                                                <ul class="nav nav-tabs pull-right">
                                                    ...
                                                    <li class="<c:if test="${(REQUEST_ACTION_TYPE eq 'DETAIL_LOCAL_PARSER_CONFIGURATION')}"><c:out value="active"></c:out></c:if>" onclick="showButtons('parser_configuration'); getDetailLocalParserConfig('<%=ControllerConstants.INIT_DETAIL_LOCAL_PARSER_CONFIG%>');" >
                                                    	<a href="#asciiConfiguration" data-toggle="tab" id="parser_configuration"><spring:message code="parsing.service.config.tab.header"></spring:message></a>
                                                    </li>
                                                    <li class="<c:if test="${(REQUEST_ACTION_TYPE eq 'DETAIL_LOCAL_PARSER_ATTRIBUTE')}"><c:out value="active" ></c:out></c:if>" onclick="showButtons('parser_attribute_configuration'); getDetailLocalParserAttrDetails('<%=ControllerConstants.INIT_DETAIL_LOCAL_PARSER_ATTRIBUTE%>');" >
                                                    	<a href="#asciiAttribute" data-toggle="tab" id="parser_attribute_configuration"><spring:message code="parsing.service.attr.list.tab.header"></spring:message></a>
                                                    </li>
                                                  </sec:authorize>  
                                                </ul>
                                                <div class="fullwidth tab-content no-padding">
                                                <sec:authorize access="hasAnyAuthority('VIEW_PARSER')">
                                                	<div class="tab-pane <c:if test="${(REQUEST_ACTION_TYPE eq 'DETAIL_LOCAL_PARSER_CONFIGURATION')}"><c:out value="active"></c:out></c:if>" id="detailLocalConfiguration">
                                                		 <c:if test="${(REQUEST_ACTION_TYPE eq 'DETAIL_LOCAL_PARSER_CONFIGURATION')}">
                                                			<jsp:include page="detailLocalParserConfiguration.jsp" ></jsp:include> 
                                                		</c:if>
                                                	</div>
                                                	
                                                	<div class="tab-pane <c:if test="${(REQUEST_ACTION_TYPE eq 'DETAIL_LOCAL_PARSER_ATTRIBUTE')}"><c:out value="active"></c:out></c:if>" id="detailLocalAttribute">
                                               			<c:if test="${(REQUEST_ACTION_TYPE eq 'DETAIL_LOCAL_PARSER_ATTRIBUTE')}">
                                               				<jsp:include page="detailLocalParserAttribute.jsp" ></jsp:include> 
                                               			</c:if>
                                                	</div>
                                                	</sec:authorize>
        	...
        	<div style="display: block;">
				...
				<form id="local-parser-parser-form" action = "" method="">
	...
	<footer class="main-footer positfix">
    	<div class="fooinn">
        	<div class="fullwidth">
       			<div class="padleft-right" id="detailLocalParserConfigDiv" style="display: none;">
            	...
            	<div class="padleft-right" id="detail_local_attribute_default" style="display: none;">
				</div>
        	</div>
        	<script>
				function showHideButtonBasedOnTabsSelected(tabType){
					$("#detailLocalParserConfigDiv").hide();
					$("#detail_local_attribute_default").hide();
					
					if(tabType == 'DETAIL_LOCAL_PARSER_CONFIGURATION'){
						$("#detailLocalParserConfigDiv").show();
					}else if(tabType == 'DETAIL_LOCAL_PARSER_ATTR_LIST'){
						$("#detail_local_attribute_default").show();
					}
				}
				
				function showButtons(id){
					if(id == 'parser_configuration'){
						showHideButtonBasedOnTabsSelected('DETAIL_LOCAL_PARSER_CONFIGURATION');
					}else if(id == 'parser_attribute_configuration'){
						showHideButtonBasedOnTabsSelected('DETAIL_LOCAL_PARSER_ATTR_LIST');
					}
				}
			...
			$(document).ready(function() {
				...
				if(parserDeviceTypeId > 0 && currentTab == 'DETAIL_LOCAL_PARSER_CONFIGURATION'){
			...
			function getDetailLocalParserConfig(action){
			
				$("#local-parser-parser-form").attr("action",action);
				$("#local-parser-parser-form").attr("method","GET");
				
				$("#local-parser-parser-form #deviceName").val($("#deviceName").find(":selected").text());
				$("#local-parser-parser-form #mappingName").val($("#mappingName").find(":selected").text());
				$("#local-parser-parser-form #mappingId").val($("#mappingName").find(":selected").val());
				$("#local-parser-parser-form #deviceId").val($("#deviceName").find(":selected").val());
				
				$("#local-parser-parser-form").submit();
			}
			
			function getDetailLocalParserAttrDetails(action){
				
				$("#local-parser-parser-form").attr("action",action);
				$("#local-parser-parser-form").attr("method","POST");
				
				$("#local-parser-parser-form #deviceName").val($("#deviceName").find(":selected").text());
				if($("#local-parser-parser-form #deviceName").val() == ''){
					$("#local-parser-parser-form #deviceName").val('${deviceName}')
				}
				
				$("#local-parser-parser-form #mappingName").val($("#mappingName").find(":selected").text());
				if($("#local-parser-parser-form #mappingName").val() == ''){
					$("#local-parser-parser-form #mappingName").val('${mappingName}');
				}
				
				$("#local-parser-parser-form #mappingId").val($("#mappingName").find(":selected").val());
				if($("#local-parser-parser-form #mappingId").val() == ''){
					$("#local-parser-parser-form #mappingId").val("${mappingId}");
				}
				$("#local-parser-parser-form #deviceId").val($("#deviceName").find(":selected").val());
				if($("#local-parser-parser-form #deviceId").val() == ''){
					$("#local-parser-parser-form #deviceId").val('${deviceId}');
				}
				$("#local-parser-parser-form").submit();
			}
			...
			function disableDLPAdvanceDetails(){
			...
			/*
			function enableDLPAdvanceDetails(){
				$("#advance_details_div_mask").remove();
				$("#advance_details_div").css("opacity","");
				
			}
			*/
			function enableDetailLocalAdvanceDetails(){
				$("#advance_details_div_mask").remove();
				$("#advance_details_div").css("opacity","");
