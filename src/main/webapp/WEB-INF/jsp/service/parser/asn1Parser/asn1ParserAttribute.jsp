...
<div class="tab-content no-padding  mtop15">
	<div class="fullwidth mtop15" id="attribute_details_div">
		<form:form modelAttribute="<%=FormBeanConstants.ASN1_PARSER_MAPPING_FORM_BEAN %>" method="POST"
		...
		<div class="fullwidth" id="add_delete_attribute_link_div">
			<div class="title2">
				...
				<span class="title2rightfield-icon1-text">
					...
					<%-- <a href="#" onclick="downloadExcel('${mappingId}','${plugInType}');">
						<spring:message code="parserConfiguration.attribute.grid.export.attribute.label" ></spring:message>
					</a>  --%>
				</span> 
				<sec:authorize access='hasAuthority(\'CREATE_PARSER\')'>
				 <span class="title2rightfield-icon1-text"> 
				...
				<span class="title2rightfield-icon1-text"> 
					<a href="#" onclick="clearAllMessages();clearAllMessagesPopUp(),openUploadDictPopup();">
						<i class="fa fa-upload"></i>
					</a> 
				    <a href="#" onclick="clearAllMessages();clearAllMessagesPopUp(),openUploadDictPopup();">
						<spring:message code="parserConfiguration.attribute.grid.uploaddictionary.label" ></spring:message>
					</a> 
				</span>
				</c:if>  
		...
		<form:form modelAttribute="parser_attribute_form_bean" method="POST"
			action="<%=ControllerConstants.ADD_EDIT_ASN1_PARSER_ATTRIBUTE%>"
		...
		<form:form modelAttribute="<%=FormBeanConstants.ASN1_PARSER_MAPPING_FORM_BEAN %>" method="POST" action="" id="asn1-parser-configuration-form">
...
<a href="#divuploadDictData" class="fancybox" style="display: none;" id="uploadDictData">#</a>
<div id="divuploadDictData" style=" width:100%; display: none;" >
	<jsp:include page="asn1UploadDictionaryPopup.jsp"></jsp:include>
</div> 
...
$(document).ready(function() {
	...
	if('${REQUEST_ACTION_TYPE}' == 'ASN1_HEADER_PARSER_ATTRIBUTE' || '${REQUEST_ACTION_TYPE}' == 'ASN1_TRAILER_PARSER_ATTRIBUTE' ){
	...
	var nextActionType = '';
	if(attributeListType == '<%= BaseConstants.ASN1_PARSER_ATTRIBUTE %>'){
		nextActionType = '<%= BaseConstants.ASN1_HEADER_PARSER_ATTRIBUTE %>';
	}else if(attributeListType == '<%= BaseConstants.ASN1_HEADER_PARSER_ATTRIBUTE %>'){
		nextActionType = '<%= BaseConstants.ASN1_TRAILER_PARSER_ATTRIBUTE %>';
	} 	
...
function addBasicDetail(){
    ...
    $.ajax({
		url: '<%=ControllerConstants.UPDATE_ASN1_PARSER_MAPPING%>',
		data : oMyForm ,
        type : "POST",
        datatype: "json",
        processData: false, 
...
function openUploadDictPopup(){
	//addColNames(gridName);
	$("#uploadDictData").click();
}
