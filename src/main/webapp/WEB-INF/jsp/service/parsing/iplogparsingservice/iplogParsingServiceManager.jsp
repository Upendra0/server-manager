												...
												<input type="hidden" id="serviceStatus" name="serviceStatus" value="">
														<ul class="nav nav-tabs pull-right">
															<sec:authorize access="hasAnyAuthority('VIEW_SERVICE_INSTANCE')">
																<li id="tab1" class="<c:if test="${(REQUEST_ACTION_TYPE eq 'IPLOG_SERVICE_SUMMARY')}"><c:out value="active"></c:out></c:if>" onclick="showButtons('iplog-parsing-service-summary');loadSummary();" >
																	<a href="#iplog-parsing-service-summary" id="a-iplog-parsing-service-summary" data-toggle="tab" aria-expanded="false"> 
																		<spring:message	code="iplog.parsing.service.summary.tab.header" ></spring:message>
																	</a>
																</li>
															</sec:authorize>
															<sec:authorize access="hasAnyAuthority('VIEW_SERVICE_INSTANCE')">
																<li id="tab2" class="<c:if test="${(REQUEST_ACTION_TYPE eq 'IPLOG_SERVICE_CONFIGURATION')}"><c:out value="active"></c:out></c:if>" onclick="showButtons('iplog-parsing-service-configuration');loadConfiguration();">
																	<a href="#iplog-parsing-service-configuration" id="a-iplog-parsing-service-configuration"	data-toggle="tab" aria-expanded="true"> 
																		<spring:message	code="iplog.parsing.service.config.tab.header" ></spring:message>
																	</a>
																</li>
															</sec:authorize>
															<sec:authorize access="hasAnyAuthority('VIEW_SERVICE_INSTANCE')">
																<li id="tab3" class="<c:if test="${(REQUEST_ACTION_TYPE eq 'IPLOG_SERVICE_HASH_CONFIGURATION')}"><c:out value="active"></c:out></c:if>" onclick="showButtons('iplog-parsing-hash-config');loadHashConfiguration();">
																	<a href="#iplog-parsing-hash-config" id="a-iplog-parsing-hash-config"	data-toggle="tab" aria-expanded="true"> 
																		<spring:message	code="iplog.parsing.service.hash.index.tab.header" ></spring:message>
																	</a>
																</li>
															</sec:authorize>
<%-- 															<sec:authorize access="hasAnyAuthority('VIEW_PATHLIST')"> --%>
																<li id="tab3" class="<c:if test="${(REQUEST_ACTION_TYPE eq 'IPLOG_SERVICE_PATHLIST_CONFIGURATION')}"><c:out value="active"></c:out></c:if>" onclick="showButtons('iplog-parsing-pathlist-config');loadPathlist();">
																	<a href="#iplog-parsing-pathlist-config" id="a-iplog-parsing-pathlist-config"	data-toggle="tab" aria-expanded="true"> 
...
<%-- 															</sec:authorize> --%>
														...
														<div class="fullwidth no-padding tab-content">
															<sec:authorize access="hasAnyAuthority('VIEW_SERVICE_INSTANCE')">
																<div id="iplog-parsing-service-summary" class="tab-pane <c:if test="${(REQUEST_ACTION_TYPE eq 'IPLOG_SERVICE_SUMMARY')}"><c:out value="active"></c:out></c:if> ">
																	<c:if test="${(REQUEST_ACTION_TYPE eq 'IPLOG_SERVICE_SUMMARY')}">
															 			<jsp:include page="../ipLogAndParsingServiceSummary.jsp"></jsp:include>
															 		</c:if>	 
																</div>
															</sec:authorize>
															<sec:authorize access="hasAnyAuthority('VIEW_SERVICE_INSTANCE')">
																<div id="iplog-parsing-service-configuration" class="tab-pane  <c:if test="${(REQUEST_ACTION_TYPE eq 'IPLOG_SERVICE_CONFIGURATION')}"><c:out value="active"></c:out></c:if>">
																	<c:if test="${(REQUEST_ACTION_TYPE eq 'IPLOG_SERVICE_CONFIGURATION')}">
 																		<jsp:include page="../ipLogAndParsingServiceConfiguration.jsp"></jsp:include> 
																	</c:if>	
																</div>
															</sec:authorize>
															<sec:authorize access="hasAnyAuthority('VIEW_SERVICE_INSTANCE')">
																<div id="iplog-parsing-hash-config" class="tab-pane  <c:if test="${(REQUEST_ACTION_TYPE eq 'IPLOG_SERVICE_HASH_CONFIGURATION')}"><c:out value="active"></c:out></c:if>">
																	<c:if test="${(REQUEST_ACTION_TYPE eq 'IPLOG_SERVICE_HASH_CONFIGURATION')}"> 
																		<jsp:include page="iplogServiceHashConfiguration.jsp"></jsp:include>
																	 </c:if>
																</div>
															</sec:authorize> 
															<sec:authorize access="hasAnyAuthority('VIEW_PATHLIST')">
																<div id="iplog-parsing-pathlist-config" class="tab-pane  <c:if test="${(REQUEST_ACTION_TYPE eq 'IPLOG_SERVICE_PATHLIST_CONFIGURATION')}"><c:out value="active"></c:out></c:if>">
																	<c:if test="${(REQUEST_ACTION_TYPE eq 'IPLOG_SERVICE_PATHLIST_CONFIGURATION')}"> 
																		<jsp:include page="iplogServicePathlist.jsp"></jsp:include>
																	 </c:if>
																</div>
															</sec:authorize> 
														</div>
		...
		<form id="iplog-parsing-service-config" method="POST" action="<%= ControllerConstants.INIT_UPDATE_IPLOG_PARSING_SERVICE_CONFIGURATION %>">
		...
		<form id="form-iplog-parsing-service-summary" method="get" action="<%= ControllerConstants.INIT_IPLOG_PARSING_SERVICE_MANAGER %>">
			<input type="hidden" name="serviceId"  value="${serviceId}"/>
			<input type="hidden" name="serviceType"  value="${serviceType}"/>
		...
		<div style="display: hidden;">
			 ...
			 <form id="iplog-parsing-service-parser-config" method="GET" action="<%=ControllerConstants.INIT_PARSER_CONFIGURATION%>">
			<input type="hidden" id="serviceId" name="serviceId" value="${serviceId}"/>
			<input type="hidden" id="serviceName" name="serviceName" value="${serviceName}"/>
			<input type="hidden" name="serviceType"  value="${serviceType}"/>
			<input type="hidden" name="instanceId"  value="${instanceId}"/>
		    	...
		    	<div class="fooinn">
		    		<div class="fullwidth">
						<div class="button-set">
		            		...
		            		<div class="padleft-right" id="hashConfigurationbtnDiv" style="display: none;">
		            			<sec:authorize access="hasAnyAuthority('UPDATE_SERVICE_INSTANCE')">
			            		<button id="btnUpdateService" class="btn btn-grey btn-xs " tabindex="21" onclick="updateHashConfigForm();">
									<spring:message code="btn.label.update" ></spring:message>
								</button>
								<button class="btn btn-grey btn-xs " tabindex="21"  onclick="loadSummary();">
									<spring:message code="btn.label.cancel" ></spring:message>
								</button>
								</sec:authorize>
								<div class="pull-right" style="font-size:10px;"><i class="fa fa-square" style="font-size: 9px"></i>&nbsp;&nbsp;<spring:message code="restart.operation.require.message" ></spring:message> </div>
		        	 ...
		        	 <script type="text/javascript">
			       		...
			       		function showButtons(tabType){
			       			...
			       			$("#summarybtnDiv").hide();
			       			$("#hashConfigurationbtnDiv").hide();
			       			$("#pathlistConfigurationbtnDiv").hide();
			       			
			       			if(tabType == 'a-iplog-parsing-service-summary'){
			       				$("#summarybtnDiv").show();
			       			}else if(tabType == 'a-iplog-parsing-service-configuration'){
			       				$("#configurationbtnDiv").show();
			       			}else if (tabType == 'a-iplog-parsing-hash-config'){
				   				$("#hashConfigurationbtnDiv").show();
			       			} else if (tabType == 'a-iplog-parsing-pathlist-config'){
				   				$("#pathlistConfigurationbtnDiv").show();
			       			}
			       			// load service status on click of each tab
			       			loadServiceStatusGUI('${serviceId}','iplog_service_summary');
			       		}
			       		...
			       		function loadConfiguration(){
			       			$('#iplog-parsing-service-config').attr("action","<%= ControllerConstants.INIT_UPDATE_IPLOG_PARSING_SERVICE_CONFIGURATION %>")
			       			$('#iplog-parsing-service-config').submit();
			       			showButtons('a-iplog-parsing-service-configuration');
			       		}
			       		
			       		function loadSummary(){
			       			$('#form-iplog-parsing-service-summary').submit();
			       			showButtons('a-iplog-parsing-service-summary');
			       		}
			       		
			       		function loadPathlist(){
			       			$('#iplog-parsing-service-config').attr("action","<%= ControllerConstants.INIT_UPDATE_IPLOG_PARSING_PATHLIST_CONFIGURATION %>")
			       			$('#iplog-parsing-service-config').submit();
			       			showButtons('a-iplog-parsing-service-configuration');
			       		}
			       		
			       		function loadHashConfiguration(){
			       			$('#iplog-parsing-service-config').attr("action","<%= ControllerConstants.INIT_UPDATE_PARSING_HASH_CONFIGURATION %>")
			       			$('#iplog-parsing-service-config').submit();
			       			showButtons('a-iplog-parsing-service-configuration');
			       		}
			       		...
			       		function redirectParserConfig(parserId,parserName,parserType){
			       			...
			       			$('#iplog-parsing-service-parser-config').submit();
