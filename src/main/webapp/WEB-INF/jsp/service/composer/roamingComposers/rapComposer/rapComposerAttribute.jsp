...
<div class="tab-content no-padding  mtop15">
	<div class="fullwidth mtop15" id="attribute_details_div">
		<form:form modelAttribute="<%=FormBeanConstants.RAP_COMPOSER_MAPPING_FORM_BEAN %>" method="POST"
	...
	<form:form modelAttribute="<%=FormBeanConstants.RAP_COMPOSER_MAPPING_FORM_BEAN %>" method="POST" action="NO_ACTION" id="ascii-composer-configuration-form">
...
$(document).ready(function() {
	...
	createRapAttributeGrid('<%=MapCache.getConfigValueAsInteger(SystemParametersConstant.TOTAL_ROWS_TO_DISPLAY_IN_GRID, 10)%>');
...
function addBasicDetail(){
    ...
    $.ajax({
		url: '<%=ControllerConstants.UPDATE_RAP_COMPOSER_MAPPING%>',
		data : oMyForm ,
        type : "POST",
        datatype: "json",
        processData: false, 
...
/*Function will create JQGRID for RAP composer attribute list */
function createRapAttributeGrid(defaultRowNum){				
	 ...
	 $("#asciiAttributeList").jqGrid({	    	
			...
			colModel:[
			          ...
			          {name: 'checkbox', index: 'checkbox',sortable:false, formatter:rapComposerAttributeCheckboxFormatter, align : 'center', width:'30%', search : false},
...
/*Function will load All RAP composer attributes to JQGRID */
 ...
 */
function rapComposerAttributeCheckboxFormatter(cellvalue, options, rowObject) {
