...
<div class="tab-content no-padding  mtop15">
	<div class="fullwidth mtop15" id="attribute_details_div">
		<form:form modelAttribute="<%=FormBeanConstants.NRTRDE_COMPOSER_MAPPING_FORM_BEAN %>" method="POST"
			...
			<div class="box box-warning" id="attribute_basic_details_div">
				...
				<div class="col-md-6 no-padding">
					...
					<div class="col-md-6 no-padding">
						<spring:message code="roaming.plugin.compose.as.single.record.label.tooltip" var="tooltip" ></spring:message>
						<div class="form-group">
							<div class="table-cell-label">${tooltip}</div>
							<div class="input-group ">
								<form:select path="composeAsSingleRecordEnable" id="composeAsSingleRecordEnable"
								...
								<spring:bind path="composeAsSingleRecordEnable">
									<elitecoreError:showError errorMessage="${status.errorMessage}"></elitecoreError:showError>
								</spring:bind>
							</div>
						</div>
		...
		<form:form modelAttribute="composer_attribute_form_bean" method="POST"
			action="<%=ControllerConstants.ADD_EDIT_NRTRDE_COMPOSER_ATTRIBUTE%>"
		...
		<div>
			<jsp:include page="../../ComposerGroupAttributeConfig.jsp"></jsp:include> <!-- All Group attribute related Stuff -->
		</div>
		
	</div>
	<form:form modelAttribute="<%=FormBeanConstants.NRTRDE_COMPOSER_MAPPING_FORM_BEAN %>" method="POST" action="NO_ACTION" id="ascii-composer-configuration-form">
...
$(document).ready(function() {
	...
	createNrtrdeAttributeGrid('<%=MapCache.getConfigValueAsInteger(SystemParametersConstant.TOTAL_ROWS_TO_DISPLAY_IN_GRID, 10)%>');
...
function addBasicDetail(){
    ...
    oMyForm.append("composeAsSingleRecordEnable", $("#composeAsSingleRecordEnable").val());
    
    $.ajax({
		url: '<%=ControllerConstants.UPDATE_NRTRDE_COMPOSER_MAPPING%>',
		data : oMyForm ,
        type : "POST",
        datatype: "json",
        processData: false, 
...
/*Function will create JQGRID for NRTRDE composer attribute list */
function createNrtrdeAttributeGrid(defaultRowNum){				
	 ...
	 $("#asciiAttributeList").jqGrid({	    	
	        ...
	        colNames:[
                      ...
                      "attributeOrder",
                      "<spring:message code='roaming.plugin.attr.composefromjson.label.tooltip' ></spring:message>",
                      "<spring:message code='roaming.plugin.attr.clonerecord.label.tooltip' ></spring:message>"
	                 ],
			colModel:[
			          {name: 'id', index: 'id',hidden:true,sortable:false,search : false},
			          {name: 'checkbox', index: 'checkbox',sortable:false, formatter:nrtrdeComposerAttributeCheckboxFormatter, align : 'center', width:'30%', search : false},
					  ...
					  {name: 'composeFromJson',index: 'composeFromJson',hidden:true, search : false},
					  {name: 'cloneRecord',index: 'cloneRecord',hidden:true, search : false}
	        ],
...
/*Function will load All NRTRDE composer attributes to JQGRID */
			...
			if(responseCode == "200"){				
				...
				if(attributeList!=null && attributeList != 'undefined' && attributeList != undefined){					
					...
					$.each(attributeList,function(index,attribute){		 				
		 				...
		 				rowData.composeFromJson		= attribute.composeFromJson;
		 				rowData.cloneRecord 		= attribute.cloneRecord;
 ...
 */
function nrtrdeComposerAttributeCheckboxFormatter(cellvalue, options, rowObject) {
