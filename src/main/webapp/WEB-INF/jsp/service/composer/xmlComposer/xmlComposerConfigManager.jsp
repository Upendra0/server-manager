...
var jsSpringMsg = {};
	...
	jsSpringMsg.xmlComposer='<%=EngineConstants.XML_COMPOSER_PLUGIN%>';
...
<%--                             		  ${serviceName}&nbsp;-&nbsp;${serviceInstanceId} --%>
                                        ...
                                        <div class="box-body table-responsive no-padding">
                                            <div class="nav-tabs-custom"> 
                                                ...
                                                <ul class="nav nav-tabs pull-right">
                                                    ...
                                                    <li class="<c:if test="${(REQUEST_ACTION_TYPE eq 'XML_COMPOSER_CONFIGURATION')}"><c:out value="active"></c:out></c:if>" onclick="showButtons('composer_configuration'); getXMLComposerConfig('<%=ControllerConstants.INIT_XML_COMPOSER_MANAGER%>');" >
                                                    	<a href="#xmlConfiguration" data-toggle="tab" id="composer_configuration"><spring:message code="parsing.service.config.tab.header"></spring:message></a>
                                                    </li>
                                                    <li class="<c:if test="${(REQUEST_ACTION_TYPE eq 'XML_COMPOSER_ATTRIBUTE')}"><c:out value="active" ></c:out></c:if>" onclick="showButtons('composer_attribute_configuration'); getXMLComposerAttrDetails('<%=ControllerConstants.INIT_XML_COMPOSER_ATTRIBUTE%>');" >
                                                    	<a href="#xmlAttribute" data-toggle="tab" id="composer_attribute_configuration"><spring:message code="parsing.service.attr.list.tab.header"></spring:message></a>
                                                    </li>
                                                  </sec:authorize>  
                                                </ul>
                                                <div class="fullwidth tab-content no-padding">
                                                <sec:authorize access="hasAnyAuthority('VIEW_COMPOSER')">
                                                	<div class="tab-pane <c:if test="${(REQUEST_ACTION_TYPE eq 'XML_COMPOSER_CONFIGURATION')}"><c:out value="active"></c:out></c:if>" id="xmlConfiguration">
                                                		 <c:if test="${(REQUEST_ACTION_TYPE eq 'XML_COMPOSER_CONFIGURATION')}">
                                                		 	<jsp:include page="xmlComposerConfiguration.jsp" ></jsp:include>
                                                		 </c:if>
                                                	</div>
                                                	
                                                	<div class="tab-pane <c:if test="${(REQUEST_ACTION_TYPE eq 'XML_COMPOSER_ATTRIBUTE')}"><c:out value="active"></c:out></c:if>" id="xmlAttribute">
                                               			<c:if test="${(REQUEST_ACTION_TYPE eq 'XML_COMPOSER_ATTRIBUTE')}">
                                               				<jsp:include page="xmlComposerAttribute.jsp" ></jsp:include>
                                               			</c:if>
                                                	</div>
                                                	</sec:authorize>
        	...
        	<div style="display: block;">
				...
				<form id="xml-composer-form" action = "" method="">
	...
	<footer class="main-footer positfix">
    	<div class="fooinn">
        	<div class="fullwidth">
       			<div class="padleft-right" id="xmlComposerConfigDiv" style="display: none;">
...
<script>
	function showHideButtonBasedOnTabsSelected(tabType){
		$("#xmlComposerConfigDiv").hide();
		$("#xml_attribute_default").hide();
		
		if(tabType == 'XML_COMPOSER_CONFIGURATION'){
			$("#xmlComposerConfigDiv").show();
		}else if(tabType == 'XML_COMPOSER_ATTR_LIST'){
	...
	function showButtons(id){
		if(id == 'composer_configuration'){
			showHideButtonBasedOnTabsSelected('XML_COMPOSER_CONFIGURATION');
		}else if(id == 'composer_attribute_configuration'){
			showHideButtonBasedOnTabsSelected('XML_COMPOSER_ATTR_LIST');
		}
	}
...
$(document).ready(function() {
	...
	if(composerDeviceTypeId > 0 && currentTab == 'XML_COMPOSER_CONFIGURATION'){
...
function getXMLComposerConfig(action){

	$("#xml-composer-form").attr("action",action);
	$("#xml-composer-form").attr("method","GET");
	
	$("#xml-composer-form #deviceName").val($("#deviceName").find(":selected").text());
	$("#xml-composer-form #mappingName").val($("#mappingName").find(":selected").text());
	$("#xml-composer-form #mappingId").val($("#mappingName").find(":selected").val());
	$("#xml-composer-form #deviceId").val($("#deviceName").find(":selected").val());
	
	$("#xml-composer-form").submit();
}

function getXMLComposerAttrDetails(action){
	
	$("#xml-composer-form").attr("action",action);
	$("#xml-composer-form").attr("method","POST");
	...
	if($("#deviceName").find(":selected").text()) {
		...
		$("#xml-composer-form #deviceName").val('${deviceName}');
	}
	if($("#mappingName").find(":selected").text()) {
		$("#xml-composer-form #mappingName").val($("#mappingName").find(":selected").text());	
	} else {
		$("#xml-composer-form #mappingName").val('${mappingName}');
	}
	if($("#mappingName").find(":selected").val()) {
		$("#xml-composer-form #mappingId").val($("#mappingName").find(":selected").val());	
	} else {
		$("#xml-composer-form #mappingId").val('${mappingId}');
	}
	if($("#deviceName").find(":selected").val()) {
		$("#xml-composer-form #deviceId").val($("#deviceName").find(":selected").val());	
	} else {
		$("#xml-composer-form #deviceId").val('${deviceId}');
	}
