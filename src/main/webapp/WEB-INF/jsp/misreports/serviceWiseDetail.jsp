...
<div class="fullwidth ">
             ...
             <div class="box-body inline-form " >
             	<div class="col-md-6 no-padding">
	             	...
	             	<spring:message code="service.wise.summary.selectServerInstance" var="selectServerInstance" ></spring:message>
	             	<div class="form-group">
	         		<div class="table-cell-label">${label}</div>
	             	<div class="input-group ">
	             		<select  name = "serverInstanceId" class="form-control table-cell input-sm"  tabindex="1" id="serverInstanceId" data-toggle="tooltip" data-placement="bottom"  title="${tooltip }" onchange="fetchServiceListByServerInstanceId()">
						<option value="-1" selected="selected">${selectServerInstance}</option>
							<c:if
								test="${SERVERINSTANCE_LIST !=null && fn:length(SERVERINSTANCE_LIST) gt 0}">
             	...
             	<div class="col-md-6 no-padding">
	         		...
	         		<div class="table-cell-label">${label }</div>
	             	<div class="input-group ">
	             		<select  name = "serviceInstanceId" class="form-control table-cell input-sm"  tabindex="1" id="serviceInstanceId" data-toggle="tooltip" data-placement="bottom"  title="${tooltip }">
   ...
   <div class="fullwidth">
	...
	'				      		</c:forEach>
     		...
     		<div class="pbottom15 form-group ">
     			<sec:authorize access="hasAuthority('GENERATE_SERVICE_WISE_DETAIL_REPORT')">
   					<button id="generateReportDetailBtn" class="btn btn-grey btn-xs" onclick="generateReport();"><spring:message code="btn.label.generatereport" ></spring:message></button>
    				<button class="btn btn-grey btn-xs" onclick="resetReportParams();"><spring:message code="btn.label.reset" ></spring:message></button>
    			</sec:authorize>
   			</div>   			
   		</div>	
      ...
      <!--   <div class="fullwidth mtop10"> -->
	   		<div class="title2">
	   			...
	   			<sec:authorize access="hasAuthority('DOWNLOAD_SERVICE_WISE_DETAIL_REPORT')">
       ...
       <!--  </div> -->
        ...
        <form id="misReport_download_form" action="" method="POST" name="misReport_download_form">
            ...
            <input type="hidden" name="serverInstanceId" id="serverInstanceId" value="" />
...
function validateServiceAndServerValues(){
	if( ($("#serverInstanceId").val()) === '-1'){
		showErrorMsg(jsSpringMsg.invalidserverInstanceId);
		return false;
	}else if($("#serviceInstanceId").val() === '-1'){
...
function fetchServiceListByServerInstanceId(){
	clearAllMessages();
	$('#serviceInstanceId option').each(function(index, option) {
	    $(option).remove();
	});
	$('#serviceInstanceId').append($('<option>', {value:-1, text:'SELECT SERVICE TYPE'}));
	var selectedSI=$("#serviceInstanceId").val();
	
	$.ajax({
		url: '<%=ControllerConstants.FETCH_SERVICELIST_BY_SI%>',
		cache: false,
		async: true, 
		dataType: 'json',
		type: "POST",
		...
		success: function(data){
			...
			if(responseCode == "200"){
				var svcListObj=eval(responseObject);
				var svcList=svcListObj['svcList'];
					if(svcList!=null && svcList != 'undefined' && svcList != undefined){
						$.each(svcList,function(index,attribute){
			 				$('#serviceInstanceId').append($('<option>', { 
...
function getServerInstanceListForPacketBasedService(){
	$.ajax({
		url: '<%=ControllerConstants.GET_SERVER_DETAIL_LIST_FOR_PACKET_BASED_SERVICE%>',
		cache: false,
		async: true,
		dataType: 'json',
		type: "POST",
	...
	function populateResultDataForPacketBasedServiceInGrid(reportData) {
	    ...
	    colNames:[
	             ...
	             "<spring:message code='serverManagement.grid.column.month' ></spring:message>",
...
function getServerInstanceListForFileBasedService(){
	$.ajax({
		url: '<%=ControllerConstants.GET_SERVER_DETAIL_LIST_FOR_FILE_BASED_SERVICE%>',
		cache: false,
		async: true,
		dataType: 'json',
		type: "POST",
...
function exportData(file){
	...
	if(validateDateParametersForDownload()){
	    ...
	    document.misReport_download_form.serverInstanceId.value=$("#serverInstanceId").val();
	    ...
	    document.misReport_download_form.action='<%=request.getContextPath()%>/<%= ControllerConstants.DOWNLOAD_MIS_REPORT_IN_DETAIL %>';  // send it to server which will open this contents in excel file
