...
<xsl:stylesheet version="1.0"
	...
	<xsl:param name="entityId" />
		<local-collection-driver>
			<xsl:for-each select="collectionService/myDrivers[id=$entityId]">
				<xsl:if test="driverType/alias = 'LOCAL_COLLECTION_DRIVER'">
								...
								<xsl:if test="status != 'DELETED'">
									<path>
										...
										<filter-pattern>
											...
											<valid-file-time-interval>
												<xsl:value-of select="validFileTimeInterval" />
											</valid-file-time-interval>
										</filter-pattern>
										<file-date>
