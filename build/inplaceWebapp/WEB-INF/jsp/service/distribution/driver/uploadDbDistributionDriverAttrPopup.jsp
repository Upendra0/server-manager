...
<!-- MED-8969 START : Attribute upload screen -->
 		  <div class="modal-content">
 				...
 				<form action="<%= ControllerConstants.DOWNLOAD_DISTRIBUTION_DB_DRIVER_ATTRIBUTE%>" method="POST" id="sample_lookup_table_form">
					<input type="hidden" id="driverId" name="driverId" value="${driverId}"/>	
					<input type="hidden" id="colNamesFromGrid" name="colNamesFromGrid"/>
					<input type="hidden" id="sampleRequired" name="sampleRequired" value='NO'/>
 ...
 <a href="#divUploadedData" class="fancybox" style="display: none;" id="uploadedData">#</a>
 		<div id="divUploadedData" style=" width:100%; display: none;" >
 			<jsp:include page="../../composer/uploadedComposerAttrDataSummary.jsp"></jsp:include>
	</div>	
...
function uploadDriverAttrPopup() {
	...
	if (!(regex.test(val))) {
		...
		uploadDriverAttrData('<%=ControllerConstants.UPLOAD_DISTRIBUTION_DB_DRIVER_ATTR_DATA%>');
	}
 }
$(document).on("change", "#dataFile", function(event) {
 ...
 function uploadDriverAttrData(urlAction){
	    ...
	    oMyForm.append("driverId", driverId);	
	   $('#processbar').show();
	   $('#buttonsDiv').hide();
