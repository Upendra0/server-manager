...
<div class="tab-content no-padding  mtop15">
	<div class="fullwidth mtop15" id="attribute_details_div">
		<form:form modelAttribute="<%=FormBeanConstants.VAR_LENGTH_ASCII_PARSER_MAPPING_FORM_BEAN %>" method="POST"
		...
		<div class="fullwidth" id="add_delete_attribute_link_div">
			<div class="title2">
				...
				<span class="title2rightfield-icon1-text">
					<a onclick="addColNames('#varLengthAsciiAttributeList');downloadCSVFile();">
						<i class="fa fa-download"></i>
					</a> 
					<a href="#" onclick="addColNames('#varLengthAsciiAttributeList');downloadCSVFile();">
						<spring:message code="parserConfiguration.attribute.grid.exportattribute.label" ></spring:message>
					</a>
				</span> 
				<sec:authorize access='hasAuthority(\'CREATE_PARSER\')'>
				<span class="title2rightfield-icon1-text"> 
					<a href="#" onclick="openUploadAttrPopup('#varLengthAsciiAttributeList');">
		...
		<div class="col-md-12 inline-form no-padding" id="attribute_grid_div">
			<div class="box-body table-responsive no-padding box"
				id="parser_attribute">
				<table class="table table-hover" id="varLengthAsciiAttributeList"></table>
				<div id="varLengthAsciiAttributeListDiv"></div>
...
<script type="text/javascript">
	...
	createVarLengthAsciiAttributeGrid('<%=MapCache.getConfigValueAsInteger(SystemParametersConstant.TOTAL_ROWS_TO_DISPLAY_IN_GRID, 10)%>');
...
function addBasicDetail(){
    ...
    $.ajax({
		url: '<%=ControllerConstants.UPDATE_VAR_LENGTH_ASCII_PARSER_MAPPING%>',
		data : oMyForm ,
        type : "POST",
        datatype: "json",
        processData: false, 
...
function getAllVarLengthAsciiAttributeListByMappingId(urlAction, mappingId){
	...
	$.ajax({
		...
		success: function(data){						
			...
			var responseObject = response.object;			
			if(responseCode == "200"){				
				$("#varLengthAsciiAttributeListDiv").show();				
				...
				if(attributeList!=null && attributeList != 'undefined' && attributeList != undefined){					
					...
					jQuery("#varLengthAsciiAttributeList").jqGrid('setGridParam', {datatype: 'local', data:gridArray }).trigger("reloadGrid");
				} 
...
function createVarLengthAsciiAttributeGrid(defaultRowNum){	
	 $("#varLengthAsciiAttributeList").jqGrid({	    	
	        datatype: "local",
	        colNames:[
	                  "#",
			...
			colModel:[
			          ...
			          {name: 'checkbox', index: 'checkbox',sortable:false, formatter:varLengthAsciiParserAttributeCheckboxFormatter, align : 'center', width:'30%', search : false},
	        ...
	        height: 'auto',
	        pager: '#varLengthAsciiAttributeListDiv',
	        viewrecords: true,
	        search:true,
	        ignoreCase: true,
	        timeout : 120000,
	 	...
	 	}).navGrid("#varLengthAsciiAttributeListDiv",{edit:false,add:false,del:false,search:true});
		$(".ui-jqgrid-titlebar").hide();
		$('#varLengthAsciiAttributeList').jqGrid('filterToolbar',{
			stringResult: true,
		...
		$("#varLengthAsciiAttributeList").sortableRows();   
		$("#varLengthAsciiAttributeList").disableSelection();
	    $("#varLengthAsciiAttributeList").sortable({
	    	items: 'tr:not(:first)'
	    });
	    
	    $('#varLengthAsciiAttributeList').navButtonAdd('#varLengthAsciiAttributeListDiv', {
           title: "Update Order",
 ...
 */
function varLengthAsciiParserAttributeCheckboxFormatter(cellvalue, options, rowObject) {
	var uniqueId = getUniqueCheckboxIdForAttribute(rowObject);
	return '<input type="checkbox" id="'+uniqueId+'" name="attributeCheckbox" value="'+rowObject["id"]+'" onchange="updateRootCheckbox(this, \'varLengthAsciiAttributeList\')"/>';
}

function updateOrder(){
	console.log("updateOrder Called");
	var ids = $('#varLengthAsciiAttributeList').jqGrid('getDataIDs');
	window.scrollTo(0, 0);
	resetWarningDisplay();
	clearAllMessages();
...
function reloadAsciiAttributeGridData(){
	...
	var $grid = $("#varLengthAsciiAttributeList");
