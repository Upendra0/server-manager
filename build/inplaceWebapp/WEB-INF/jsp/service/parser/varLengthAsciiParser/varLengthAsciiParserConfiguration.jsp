...
<div class="tab-content no-padding  mtop15">
	<div class="fullwidth mtop15">
		...
		<!-- Device details Section end here    -->
			...
			<form:form modelAttribute="<%=FormBeanConstants.VAR_LENGTH_ASCII_PARSER_MAPPING_FORM_BEAN %>" method="POST" action="" id="var-length-ascii-parser-configuration-form">
			...
			<div class="fullwidth" id="advance_details_div">
			 <jsp:include page="varLengthAsciiAdvanceDetail.jsp"></jsp:include>
			</div> 
			<!-- parser Advance details end here -->
			
			<div class="fullwidth" id="data_detfinition_div">
				<jsp:include page="varLengthAsciiDataDefinition.jsp"></jsp:include>
			</div>
...
$(document).on("change","#fileHeaderParser",function(event) {
	onFileHeaderParserChange();
});

function onFileHeaderParserChange() {
	var fileHeaderParserValue = $("#fileHeaderParser").val();
	if(fileHeaderParserValue && fileHeaderParserValue == "BAF") {
		$('#fileHeaderContainsFields').prop('disabled', 'disabled');
		$('#fileHeaderContainsFields').val('false');
	} else {
		$('#fileHeaderContainsFields').prop('disabled', false);
		//$('#fileHeaderContainsFields').val('true');
	}
}
...
function onMappingNameChange() {
	resetWarningDisplay();
   	...
   	getDataDefinitionFileName(mappingId,parserType);
}
...
function setVarLengthAsciiParserAdvanceDetail(responseObject){
	
	var separatorFind=false;
	$("#fileTypeEnum").val(responseObject.fileTypeEnum);
	showRelatedFileTypeParam();
	$("#find").val(responseObject.find);
	$("#replace").val(responseObject.replace);
	var dataDefinitionPath = responseObject.dataDefinitionPath;
	if(dataDefinitionPath!=null && dataDefinitionPath.indexOf("/dictionary")!=-1){
		  dataDefinitionPath = dataDefinitionPath.substring(dataDefinitionPath.indexOf("/dictionary"));
		  $("#dataDefinitionPath").val(dataDefinitionPath);
	}
	if(responseObject.fileTypeEnum == '<%=BaseConstants.KEY_VALUE_RECORD%>'){
		
		<c:forEach var="SeparatorEnum" items="${SeparatorEnum}">
			if(responseObject.keyValueSeparator == '${SeparatorEnum.value}'){
				$("#keyValueSepValues").val(responseObject.keyValueSeparator);
				$("#fieldSeparator").val('');
		...
		if(separatorFind == false){
			if(responseObject.keyValueSeparator == ' '){
				$("#keyValueSepValues").val('s');
				$("#fieldSeparator").val('');
				$("#keyValueSeparator").attr('readOnly',true);
			}else{
				$("#keyValueSepValues").val('Other');
				$("#keyValueSeparator").val(responseObject.keyValueSeparator);
		...
		<c:forEach var="SeparatorEnum" items="${SeparatorEnum}">
		if(responseObject.fieldSeparator == '${SeparatorEnum.value}'){
			$("#fieldSepValues").val(responseObject.fieldSeparator);
			$("#fieldSeparator").val('');
		...
		if(separatorFind == false){
			if(responseObject.fieldSeparator == ' '){
				$("#fieldSepValues").val('s');
				$("#fieldSeparator").val('');
				$("#fieldSeparator").attr('readOnly',true);
			}else{
				$("#fieldSepValues").val('Other');
				$("#fieldSeparator").val(responseObject.fieldSeparator);
				$("#fieldSeparator").attr('readOnly',false);
			}
		}
		
		
	}else if(responseObject.fileTypeEnum == '<%=BaseConstants.RECORD_HEADER%>'){
	
		<c:forEach var="SeparatorEnum" items="${SeparatorEnum}">
		if(responseObject.recordHeaderSeparator == '${SeparatorEnum.value}'){
			$("#recordHeaderSepEnum").val(responseObject.recordHeaderSeparator);
		...
		if(separatorFind == false){
			if(responseObject.recordHeaderSeparator == ' '){
				$("#recordHeaderSepEnum").val('s');
				$("#recordHeaderSeparator").val('');
				$("#recordHeaderSeparator").attr('readOnly',true);
			}else{
				$("#recordHeaderSepEnum").val('Other');
				$("#recordHeaderSeparator").val(responseObject.recordHeaderSeparator);
				$("#recordHeaderSeparator").attr('readOnly',false);
			}
		}
		
		$("#recordHeaderLength").val(responseObject.recordHeaderLength);
		
	}else if(responseObject.fileTypeEnum == '<%=BaseConstants.FILE_HEADER_FOOTER%>'){
		
		$("#fileHeaderEnable").val(responseObject.fileHeaderEnable.toString());
		...
		$("#fileHeaderParser").val(responseObject.fileHeaderParser);
		
		$("#fileHeaderContainsFields").val(responseObject.fileHeaderContainsFields.toString());
		
		$("#fileFooterEnable").val(responseObject.fileFooterEnable.toString());
		
		$("#fileFooterParser").val(responseObject.fileFooterParser);
		$("#fileFooterContains").val(responseObject.fileFooterContains);
		
	}else if(responseObject.fileTypeEnum == '<%=BaseConstants.DELIMITER%>'){
	...
	}else if(responseObject.fileTypeEnum == '<%=BaseConstants.LINEAR_KEY_VALUE_RECORD%>'){
		...
		$("#fileFooterParser").val(responseObject.fileFooterParser);
		$("#fileFooterContains").val(responseObject.fileFooterContains);
		$("#recordHeaderIdentifierId").val(responseObject.recordHeaderIdentifier);
		$("#excludeLinesStart").val(responseObject.excludeLinesStart);
		$("#excludeCharactersMin").val(responseObject.excludeCharactersMin);
		$("#excludeCharactersMax").val(responseObject.excludeCharactersMax);
