...
<div class="tab-content no-padding  mtop15">
	<div class="fullwidth mtop15" id="attribute_details_div">
		<form:form modelAttribute="<%=FormBeanConstants.PDF_PARSER_MAPPING_FORM_BEAN%>" method="POST"
			id="pdf_attribute_configuration_form_bean">
		...
		<div class="fullwidth" id="add_delete_attribute_link_div">
			<div class="title2">
				...
				<span class="title2rightfield-icon1-text">
					<a onclick="addColNames('#pdfAttributeList');downloadCSVFile();">
						<i class="fa fa-download"></i>
					</a> 
					<a href="#" onclick="addColNames('#pdfAttributeList');downloadCSVFile();">
						<spring:message code="parserConfiguration.attribute.grid.exportattribute.label" ></spring:message>
					</a>
				</span> 
				<sec:authorize access='hasAuthority(\'CREATE_PARSER\')'>
				<span class="title2rightfield-icon1-text"> 
					<a href="#" onclick="openUploadAttrPopup('#pdfAttributeList');">
		...
		<div class="col-md-12 inline-form no-padding" id="attribute_grid_div">
			<div class="box-body table-responsive no-padding box"
				id="parser_attribute">
				<table class="table table-hover" id="pdfAttributeList"></table>
				<div id="pdfAttributeListDiv"></div>
		...
		<div class=" clearfix fooinn">
        	<div class="fullwidth">
       			<div class="buttons-div" id="saveAndNext" >
        		<sec:authorize access="hasAuthority('UPDATE_PARSER')"> 
					<button class="btn btn-grey btn-xs " id="parser_association_save_next_btn"  type="button" onclick=" getPDFParserGroupAttrDetails('<%=ControllerConstants.INIT_PDF_PARSER_GROUP_ATTRIBUTE%>', 'false');"><spring:message code="btn.label.save.next"></spring:message></button>
                </sec:authorize> 
     	</div>
...
$(document).ready(function() {
	...
	createPDFAttributeGrid('<%=MapCache.getConfigValueAsInteger(SystemParametersConstant.TOTAL_ROWS_TO_DISPLAY_IN_GRID, 10)%>');
...
function addBasicDetail(){
    ...
    $.ajax({
		url: '<%=ControllerConstants.UPDATE_PDF_PARSER_MAPPING%>',
		data : oMyForm ,
        type : "POST",
        datatype: "json",
        processData: false, 
...
function createPDFAttributeGrid(defaultRowNum){				
	 
	 $("#pdfAttributeList").jqGrid({	    	
	        datatype: "local",
	        colNames:[
	                  "#",
	                  ...
	                  jsSpringMsg.location,
	                  jsSpringMsg.columnStartLocation,
	                  jsSpringMsg.columnIdentifier,
	                  jsSpringMsg.referenceRow,
	                  jsSpringMsg.pageNumber,
	                  jsSpringMsg.columnEndsWith,
	                  jsSpringMsg.valueSeparator,
	                  jsSpringMsg.mandatory,
	                  jsSpringMsg.multipleValues,
			...
			colModel:[
					  ...
					  {name: 'location',index: 'location',hidden:true,search : false},
					  {name: 'columnStartLocation',index: 'columnStartLocation',hidden:true,search : false},
					  {name: 'columnIdentifier',index: 'columnIdentifier',hidden:true,search : false},
					  {name: 'referenceRow',index: 'referenceRow',hidden:true,search : false},
					  {name: 'pageNumber',index: 'pageNumber',hidden:true,search : false},
					  {name: 'columnEndsWith',index: 'columnEndsWith',hidden:true,search : false},
					  {name: 'valueSeparator',index: 'valueSeparator',hidden:true,search : false},
					  {name: 'mandatory',index: 'mandatory',hidden:true,search : false},
					  {name: 'multipleValues',index: 'multipleValues',hidden:true,search : false},
					  {name: 'columnStartsWith',index: 'columnStartsWith',hidden:true,search : false},
	 	...
	 	}).navGrid("#pdfAttributeListDiv",{edit:false,add:false,del:false,search:true});
		$(".ui-jqgrid-titlebar").hide();
		$('#pdfAttributeList').jqGrid('filterToolbar',{
			stringResult: true,
		...
		$("#pdfAttributeList").sortableRows();   
		$("#pdfAttributeList").disableSelection();
	    $("#pdfAttributeList").sortable({
	    	items: 'tr:not(:first)'
	    });
	    
	    $('#pdfAttributeList').navButtonAdd('#pdfAttributeListDiv', {
	       title: "Update Order",
...
function updateOrder(){
	...
	var ids = $('#pdfAttributeList').jqGrid('getDataIDs');
	window.scrollTo(0, 0);
	resetWarningDisplay();
	clearAllMessages();
...
function getAllPDFAttributeListByMappingId(urlAction, mappingId){
			...
			if(responseCode == "200"){				
				...
				if(attributeList!=null && attributeList != 'undefined' && attributeList != undefined){					
					...
					$.each(attributeList,function(index,attribute){	
		 				...
		 				rowData.pageNumber   		= attribute.pageNumber;
		 				rowData.columnEndsWith   	= attribute.columnEndsWith;
		 				rowData.valueSeparator   	= attribute.valueSeparator;
		 				rowData.mandatory   		= attribute.mandatory;
		 				rowData.multipleValues 		= attribute.multipleValues;
		 				
		 				rowData.columnStartsWith   	= attribute.columnStartsWith;
			 				
			 			gridArray.push(rowData);
		 			});
					jQuery("#pdfAttributeList").jqGrid('setGridParam', {datatype: 'local', data:gridArray }).trigger("reloadGrid");
				} 
...
function reloadPDFAttributeGridData(){
	clearAllMessages();
	
	var $grid = $("#pdfAttributeList");
