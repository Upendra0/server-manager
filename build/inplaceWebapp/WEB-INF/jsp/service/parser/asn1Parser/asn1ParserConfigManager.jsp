                                        ...
                                        <div class="box-body table-responsive no-padding">
                                            <div class="nav-tabs-custom"> 
                                                ...
                                                <ul class="nav nav-tabs pull-right">
                                                    ...
                                                    <li class="<c:if test="${(REQUEST_ACTION_TYPE eq 'ASN1_PARSER_CONFIGURATION')}"><c:out value="active"></c:out></c:if>" onclick="showButtons('parser_configuration'); getAsn1ParserConfig('<%=ControllerConstants.INIT_ASN1_PARSER_CONFIG%>');" >
                                                    ...
                                                    <li class="<c:if test="${(REQUEST_ACTION_TYPE eq 'ASN1_PARSER_ATTRIBUTE')}"><c:out value="active" ></c:out></c:if>" onclick="showButtons('parser_attribute_configuration'); getAsn1ParserAttrDetails('<%=ControllerConstants.INIT_ASN1_PARSER_ATTRIBUTE%>'); " >
                                                     ...
                                                     <li class="<c:if test="${(REQUEST_ACTION_TYPE eq 'ASN1_HEADER_PARSER_ATTRIBUTE')}"><c:out value="active" ></c:out></c:if>" onclick="showButtons('parser_header_attribute_configuration'); getAsn1ParserAttrDetails('<%=ControllerConstants.INIT_ASN1_PARSER_ATTRIBUTE%>','ASN1_HEADER_PARSER_ATTRIBUTE');" >
                                                    	<a href="#asn1HeaderAttribute" data-toggle="tab" id="parser_header_attribute_configuration"><spring:message code="parsing.service.attr.header.list.tab.header"></spring:message></a>
                                                    </li> 
                                                    <li class="<c:if test="${(REQUEST_ACTION_TYPE eq 'ASN1_TRAILER_PARSER_ATTRIBUTE')}"><c:out value="active" ></c:out></c:if>" onclick="showButtons('parser_trailer_attribute_configuration'); getAsn1ParserAttrDetails('<%=ControllerConstants.INIT_ASN1_PARSER_ATTRIBUTE%>','ASN1_TRAILER_PARSER_ATTRIBUTE');" >
                                                    	<a href="#asn1TrailerAttribute" data-toggle="tab" id="parser_trailer_attribute_configuration"><spring:message code="parsing.service.attr.trailer.list.tab.header"></spring:message></a>
                                                    </li>
                                                  </sec:authorize>  
                                                </ul>
                                                <div class="fullwidth tab-content no-padding">
                                                <sec:authorize access="hasAnyAuthority('VIEW_PARSER')">
                                                	<div class="tab-pane <c:if test="${(REQUEST_ACTION_TYPE eq 'ASN1_PARSER_CONFIGURATION')}"><c:out value="active"></c:out></c:if>" id="asn1Configuration">
                                                		 <c:if test="${(REQUEST_ACTION_TYPE eq 'ASN1_PARSER_CONFIGURATION')}">
                                                			<jsp:include page="asn1ParserConfiguration.jsp" ></jsp:include>
                                                		</c:if>
                                                	</div>
                                                	
                                                	<div class="tab-pane <c:if test="${(REQUEST_ACTION_TYPE eq 'ASN1_PARSER_ATTRIBUTE')}"><c:out value="active"></c:out></c:if>" id="asn1Attribute">
                                               			<c:if test="${(REQUEST_ACTION_TYPE eq 'ASN1_PARSER_ATTRIBUTE')}">
                                               				<jsp:include page="asn1ParserAttribute.jsp" ></jsp:include>
                                               			</c:if>
                                                	</div>
                                                	<div class="tab-pane <c:if test="${(REQUEST_ACTION_TYPE eq 'ASN1_HEADER_PARSER_ATTRIBUTE')}"><c:out value="active"></c:out></c:if>" id="asn1HeaderAttribute">
                                               			<c:if test="${(REQUEST_ACTION_TYPE eq 'ASN1_HEADER_PARSER_ATTRIBUTE')}">
                                               				<jsp:include page="asn1ParserAttribute.jsp" ></jsp:include>
                                               			</c:if>
                                                	</div>
                                                	<div class="tab-pane <c:if test="${(REQUEST_ACTION_TYPE eq 'ASN1_TRAILER_PARSER_ATTRIBUTE')}"><c:out value="active"></c:out></c:if>" id="asn1TrailerAttribute">
                                               			<c:if test="${(REQUEST_ACTION_TYPE eq 'ASN1_TRAILER_PARSER_ATTRIBUTE')}">
                                               				<jsp:include page="asn1ParserAttribute.jsp" ></jsp:include>
                                               			</c:if>
                                                	</div>
                                                	</sec:authorize>
    	...
    	<div class="fooinn">
        	<div class="fullwidth">
				...
				<div class="padleft-right"  id="asn1ParserConfigbtnDiv" style="display: none;">
					<c:if test="${(REQUEST_ACTION_TYPE ne 'ASN1_TRAILER_PARSER_ATTRIBUTE') && (readOnlyFlag eq 'false') }">
...
<script>
	...
	function showHideButtonBasedOnTabsSelected(tabType){
		...
		if(tabType == 'ASN1_PARSER_CONFIGURATION'){
			$("#asn1ParserConfigDiv").show();
		}else if(tabType == 'ASN1_PARSER_ATTR_LIST'){
			$("#asn1_attribute_default").show();
			$("#asn1ParserConfigbtnDiv").show();
		}else if(tabType == 'ASN1_PARSER_HEADER_ATTR_LIST'){
			$("#asn1_header_attribute_default").show();
			$("#asn1ParserConfigbtnDiv").show();	
		}else if(tabType == 'ASN1_PARSER_TRAILER_ATTR_LIST'){
	...
	function showButtons(id){
		if(id == 'parser_configuration'){
			showHideButtonBasedOnTabsSelected('ASN1_PARSER_CONFIGURATION');
			...
			showHideButtonBasedOnTabsSelected('ASN1_PARSER_HEADER_ATTR_LIST');
		}else if(id == 'parser_trailer_attribute_configuration'){
			showHideButtonBasedOnTabsSelected('ASN1_PARSER_TRAILER_ATTR_LIST');
		}
	}
...
$(document).ready(function() {
	...
	if(parserDeviceTypeId > 0 && currentTab == 'ASN1_PARSER_CONFIGURATION'){
...
function getAsn1ParserConfig(action){
...
function getAsn1ParserAttrDetails(action,attributeType){
...
function setReadOnlyFlagWithUpdateActionType(){
	...
	$("#asn1-parser-configuration-form").attr("action",'updateAsn1ParserConfiguration');
	$("#asn1-parser-configuration-form").submit();
}
