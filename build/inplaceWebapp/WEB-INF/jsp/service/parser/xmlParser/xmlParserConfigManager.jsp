...
<div class="wrapper"> 
    ...
    <div class="content-wrapper"> 
        ...
        <section class="content-header">
                                        ...
                                        <div class="box-body table-responsive no-padding">
                                            <div class="nav-tabs-custom"> 
                                                ...
                                                <ul class="nav nav-tabs pull-right">
                                                    ...
                                                    <li class="<c:if test="${(REQUEST_ACTION_TYPE eq 'XML_PARSER_CONFIGURATION')}"><c:out value="active"></c:out></c:if>" onclick="showButtons('parser_configuration'); getXMLParserConfig('<%=ControllerConstants.INIT_XML_PARSER_CONFIG%>');" >
                                                    	<a href="#xmlConfiguration" data-toggle="tab" id="parser_configuration"><spring:message code="parsing.service.config.tab.header"></spring:message></a>
                                                    </li>
                                                    <li class="<c:if test="${(REQUEST_ACTION_TYPE eq 'XML_PARSER_ATTRIBUTE')}"><c:out value="active" ></c:out></c:if>" onclick="showButtons('parser_attribute_configuration'); getXMLParserAttrDetails('<%=ControllerConstants.INIT_XML_PARSER_ATTRIBUTE%>');" >
                                                    	<a href="#xmlAttribute" data-toggle="tab" id="parser_attribute_configuration"><spring:message code="parsing.service.attr.list.tab.header"></spring:message></a>
                                                    </li>
                                                  </sec:authorize>  
                                                </ul>
                                                <div class="fullwidth tab-content no-padding">
                                                <sec:authorize access="hasAnyAuthority('VIEW_PARSER')">
                                                	<div class="tab-pane <c:if test="${(REQUEST_ACTION_TYPE eq 'XML_PARSER_CONFIGURATION')}"><c:out value="active"></c:out></c:if>" id="xmlConfiguration">
                                                		 <c:if test="${(REQUEST_ACTION_TYPE eq 'XML_PARSER_CONFIGURATION')}">
                                                			<jsp:include page="xmlParserConfiguration.jsp" ></jsp:include>
                                                		</c:if>
                                                	</div>
                                                	
                                                	<div class="tab-pane <c:if test="${(REQUEST_ACTION_TYPE eq 'XML_PARSER_ATTRIBUTE')}"><c:out value="active"></c:out></c:if>" id="xmlAttribute">
                                               			<c:if test="${(REQUEST_ACTION_TYPE eq 'XML_PARSER_ATTRIBUTE')}">
                                               				<jsp:include page="xmlParserAttribute.jsp" ></jsp:include>
                                               			</c:if>
                                                	</div>
                                                	</sec:authorize>
        	...
        	<div style="display: block;">
				...
				<form id="xml-parser-form" action = "" method="">
	...
	<footer class="main-footer positfix">
    	<div class="fooinn">
        	<div class="fullwidth">
       			<div class="padleft-right" id="xmlParserConfigDiv" style="display: none;">
            	...
            	<div class="padleft-right" id="xml_attribute_default" style="display: none;">
				</div>
        	</div>
...
<script>
	function showHideButtonBasedOnTabsSelected(tabType){
		$("#xmlParserConfigDiv").hide();
		$("#xml_attribute_default").hide();
		
		if(tabType == 'XML_PARSER_CONFIGURATION'){
			$("#xmlParserConfigDiv").show();
		}else if(tabType == 'XML_PARSER_ATTR_LIST'){
			$("#xml_attribute_default").show();
		}
	}
	
	function showButtons(id){
		if(id == 'parser_configuration'){
			showHideButtonBasedOnTabsSelected('XML_PARSER_CONFIGURATION');
		}else if(id == 'parser_attribute_configuration'){
			showHideButtonBasedOnTabsSelected('XML_PARSER_ATTR_LIST');
		}
	}
...
$(document).ready(function() {
	...
	if(parserDeviceTypeId > 0 && currentTab == 'XML_PARSER_CONFIGURATION'){
...
function getXMLParserConfig(action){

	$("#xml-parser-form").attr("action",action);
	$("#xml-parser-form").attr("method","GET");
	
	$("#xml-parser-form #deviceName").val($("#deviceName").find(":selected").text());
	$("#xml-parser-form #mappingName").val($("#mappingName").find(":selected").text());
	$("#xml-parser-form #mappingId").val($("#mappingName").find(":selected").val());
	$("#xml-parser-form #deviceId").val($("#deviceName").find(":selected").val());
	
	$("#xml-parser-form").submit();
}

function getXMLParserAttrDetails(action){
	
	$("#xml-parser-form").attr("action",action);
	$("#xml-parser-form").attr("method","POST");
	
	$("#xml-parser-form #deviceName").val($("#deviceName").find(":selected").text());
	if($("#xml-parser-form #deviceName").val() == ''){
		$("#xml-parser-form #deviceName").val('${deviceName}')
	}
	
	$("#xml-parser-form #mappingName").val($("#mappingName").find(":selected").text());
	if($("#xml-parser-form #mappingName").val() == ''){
		$("#xml-parser-form #mappingName").val('${mappingName}');
	}
	
	$("#xml-parser-form #mappingId").val($("#mappingName").find(":selected").val());
	if($("#xml-parser-form #mappingId").val() == ''){
		$("#xml-parser-form #mappingId").val("${mappingId}");
	}
	
	$("#xml-parser-form #deviceId").val($("#deviceName").find(":selected").val());
	if($("#xml-parser-form #deviceId").val() == ''){
		$("#xml-parser-form #deviceId").val('${deviceId}');
	}
	$("#xml-parser-form").submit();
}
...
function disableXMLAdvanceDetails(){
...
function enableXMLAdvanceDetails(){
	$("#advance_details_div_mask").remove();
