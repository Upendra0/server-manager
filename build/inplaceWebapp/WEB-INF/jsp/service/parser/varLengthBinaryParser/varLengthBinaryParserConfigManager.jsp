...
var jsSpringMsg = {};
	...
	jsSpringMsg.sourceFieldName="<spring:message code='parsing.service.attr.grid.column.source.field.name' ></spring:message>";
	...
	jsSpringMsg.varLengthAsciiParser='<%=EngineConstants.VARIABLE_LENGTH_ASCII_PARSING_PLUGIN%>';
	jsSpringMsg.varLengthBinaryParser='<%=EngineConstants.VARIABLE_LENGTH_BINARY_PARSING_PLUGIN%>';
...
<div class="wrapper"> 
    ...
    <div class="content-wrapper"> 
        ...
        <section class="content-header">
                                        ...
                                        <div class="box-body table-responsive no-padding">
                                            <div class="nav-tabs-custom"> 
                                                ...
                                                <ul class="nav nav-tabs pull-right">
                                                    ...
                                                    <li class="<c:if test="${(REQUEST_ACTION_TYPE eq 'VAR_LENGTH_BINARY_PARSER_CONFIGURATION')}"><c:out value="active"></c:out></c:if>" onclick="showButtons('parser_configuration'); getVarLengthBinaryParserConfig('<%=ControllerConstants.INIT_VAR_LENGTH_BINARY_PARSER_CONFIG%>');" >
                                                    	<a href="#varLengthBinaryConfiguration" data-toggle="tab" id="parser_configuration"><spring:message code="parsing.service.config.tab.header"></spring:message></a>
                                                    </li>
                                                    <li class="<c:if test="${(REQUEST_ACTION_TYPE eq 'VAR_LENGTH_BINARY_PARSER_ATTRIBUTE')}"><c:out value="active" ></c:out></c:if>" onclick="showButtons('parser_attribute_configuration'); getVarLengthBinaryParserAttrDetails('<%=ControllerConstants.INIT_VAR_LENGTH_BINARY_PARSER_ATTRIBUTE%>');" >
                                                    	<a href="#varLengthBinaryAttribute" data-toggle="tab" id="parser_attribute_configuration"><spring:message code="parsing.service.attr.list.tab.header"></spring:message></a>
                                                    </li>
                                                    <li class="<c:if test="${(REQUEST_ACTION_TYPE eq 'VAR_LENGTH_BINARY_HEADER_PARSER_ATTRIBUTE')}"><c:out value="active" ></c:out></c:if>" onclick="showButtons('parser_header_attribute_configuration'); getVarLengthBinaryParserAttrDetails('<%=ControllerConstants.INIT_VAR_LENGTH_BINARY_PARSER_ATTRIBUTE%>','VAR_LENGTH_BINARY_HEADER_PARSER_ATTRIBUTE');" >
                                                    	<a href="#varLengthBinaryHeaderAttribute" data-toggle="tab" id="parser_header_attribute_configuration"><spring:message code="parsing.service.attr.header.list.tab.header"></spring:message></a>
                                                    </li>
                                                  </sec:authorize>  
                                                </ul>
                                                <div class="fullwidth tab-content no-padding">
                                                <sec:authorize access="hasAnyAuthority('VIEW_PARSER')">
                                                	<div class="tab-pane <c:if test="${(REQUEST_ACTION_TYPE eq 'VAR_LENGTH_BINARY_PARSER_CONFIGURATION')}"><c:out value="active"></c:out></c:if>" id="varLengthBinaryConfiguration">
                                                		 <c:if test="${(REQUEST_ACTION_TYPE eq 'VAR_LENGTH_BINARY_PARSER_CONFIGURATION')}">
                                                			<jsp:include page="varLengthBinaryParserConfiguration.jsp" ></jsp:include>
                                                		</c:if>
                                                	</div>
                                                	
                                                	<div class="tab-pane <c:if test="${(REQUEST_ACTION_TYPE eq 'VAR_LENGTH_BINARY_PARSER_ATTRIBUTE')}"><c:out value="active"></c:out></c:if>" id="varLengthBinaryAttribute">
                                               			<c:if test="${(REQUEST_ACTION_TYPE eq 'VAR_LENGTH_BINARY_PARSER_ATTRIBUTE')}">
                                               				<jsp:include page="varLengthBinaryParserAttribute.jsp" ></jsp:include>
                                               			</c:if>
                                                	</div>
                                                	
                                                	<div class="tab-pane <c:if test="${(REQUEST_ACTION_TYPE eq 'VAR_LENGTH_BINARY_HEADER_PARSER_ATTRIBUTE')}"><c:out value="active"></c:out></c:if>" id="varLengthBinaryHeaderAttribute">
                                               			<c:if test="${(REQUEST_ACTION_TYPE eq 'VAR_LENGTH_BINARY_HEADER_PARSER_ATTRIBUTE')}">
                                               				<jsp:include page="varLengthBinaryParserAttribute.jsp" ></jsp:include>
                                               			</c:if>
                                                	</div>
        	...
        	<div style="display: block;">
				...
				<form id="var-length-binary-parser-form" action = "" method="">
	...
	<footer class="main-footer positfix">
    	<div class="fooinn">
        	<div class="fullwidth">
       			<div class="padleft-right" id="varLengthBinaryParserConfigDiv" style="display: none;">
				...
				<div class="padleft-right"  id="varLengthBinaryParserConfigbtnDiv" style="display: none;">
					<c:if test="${(REQUEST_ACTION_TYPE ne 'VAR_LENGTH_BINARY_HEADER_PARSER_ATTRIBUTE') && (readOnlyFlag eq 'false') }">
            	...
            	<div class="padleft-right" id="var_length_binary_attribute_default" style="display: none;">
				</div>
				<div class="padleft-right" id="var_length_binary_header_attribute_default" style="display: none;">
				</div>
        	</div>
        	<script>
				function showHideButtonBasedOnTabsSelected(tabType){
					$("#varLengthBinaryParserConfigDiv").hide();
					$("#var_length_binary_attribute_default").hide();
					$("#var_length_binary_header_attribute_default").hide();
					$("#varLengthBinaryParserConfigbtnDiv").hide();	
					if(tabType == 'VAR_LENGTH_BINARY_PARSER_CONFIGURATION'){
						$("#varLengthBinaryParserConfigDiv").show();
					}else if(tabType == 'VAR_LENGTH_BINARY_PARSER_ATTR_LIST'){
						$("#var_length_binary_attribute_default").show();
						$("#varLengthBinaryParserConfigbtnDiv").show();	
					}else if(tabType == 'VAR_LENGTH_BINARY_PARSER_HEADER_ATTR_LIST'){
						$("#var_length_binary_header_attribute_default").show();
						$("#varLengthBinaryParserConfigbtnDiv").show();	
					}
				}
				...
				function showButtons(id){
					if(id == 'parser_configuration'){
						showHideButtonBasedOnTabsSelected('VAR_LENGTH_BINARY_PARSER_CONFIGURATION');
					}else if(id == 'parser_attribute_configuration'){
						showHideButtonBasedOnTabsSelected('VAR_LENGTH_BINARY_PARSER_ATTR_LIST');
					}else if(id == 'parser_header_attribute_configuration'){
						showHideButtonBasedOnTabsSelected('VAR_LENGTH_BINARY_PARSER_HEADER_ATTR_LIST');
					}
				}
			...
			$(document).ready(function() {
				...
				if(readOnlyFlag == '' || readOnlyFlag ==true){
					disableVarLengthBinaryAdvanceDetails();	
					disableVarLengthBinaryDataDefinition();
				}
				...
				if(parserDeviceTypeId > 0 && currentTab == 'VAR_LENGTH_BINARY_PARSER_CONFIGURATION'){
			...
			function getVarLengthBinaryParserConfig(action){
			
				$("#var-length-binary-parser-form").attr("action",action);
				$("#var-length-binary-parser-form").attr("method","GET");
				
				$("#var-length-binary-parser-form #deviceName").val($("#deviceName").find(":selected").text());
				$("#var-length-binary-parser-form #mappingName").val($("#mappingName").find(":selected").text());
				$("#var-length-binary-parser-form #mappingId").val($("#mappingName").find(":selected").val());
				$("#var-length-binary-parser-form #deviceId").val($("#deviceName").find(":selected").val());
				
				$("#var-length-binary-parser-form").submit();
			}
			
			function getVarLengthBinaryParserAttrDetails(action, attributeType){
				
				$("#var-length-binary-parser-form").attr("action",action);
				$("#var-length-binary-parser-form").attr("method","POST");
				$("#var-length-binary-parser-form #attributeType").val(attributeType);
				$("#var-length-binary-parser-form #deviceName").val($("#deviceName").find(":selected").text());
				if($("#var-length-binary-parser-form #deviceName").val() == ''){
					$("#var-length-binary-parser-form #deviceName").val('${deviceName}')
				}
				
				$("#var-length-binary-parser-form #mappingName").val($("#mappingName").find(":selected").text());
				if($("#var-length-binary-parser-form #mappingName").val() == ''){
					$("#var-length-binary-parser-form #mappingName").val('${mappingName}');
				}
				
				$("#var-length-binary-parser-form #mappingId").val($("#mappingName").find(":selected").val());
				if($("#var-length-binary-parser-form #mappingId").val() == ''){
					$("#var-length-binary-parser-form #mappingId").val("${mappingId}");
				}
				$("#var-length-binary-parser-form #deviceId").val($("#deviceName").find(":selected").val());
				if($("#var-length-binary-parser-form #deviceId").val() == ''){
					$("#var-length-binary-parser-form #deviceId").val('${deviceId}');
				}
				$("#var-length-binary-parser-form").submit();
			}
			...
			function disableVarLengthBinaryAdvanceDetails(){
			...
			function enableVarLengthBinaryAdvanceDetails(){
				$("#advance_details_div_mask").remove();
			...
			/* Function will disable the content */
			function disableVarLengthBinaryDataDefinition(){
				$("#data_detfinition_div_mask").remove();
				$('#data_detfinition_div').fadeTo('slow',.6);
				$element = $('#data_detfinition_div');
				var top = Math.ceil($element.position().top);
				var elementStr = "<div id='data_detfinition_div_mask' style='position: absolute;top:"+top+"px;left:0;width: 100%;height:100%;z-index:2;opacity:0.4;filter: alpha(opacity = 50)'></div>";
				$element.append(elementStr);
				
			}
			
			/*Function will enable the mapping details*/
			function enableVarLengthBinaryDataDefinition(){
				$("#data_detfinition_div_mask").remove();
				$("#data_detfinition_div").css("opacity","");
			}
