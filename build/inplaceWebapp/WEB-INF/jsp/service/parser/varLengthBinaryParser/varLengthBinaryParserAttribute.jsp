...
<div class="tab-content no-padding  mtop15">
	<div class="fullwidth mtop15" id="attribute_details_div">
		<form:form modelAttribute="<%=FormBeanConstants.VAR_LENGTH_BINARY_PARSER_MAPPING_FORM_BEAN %>" method="POST"
		...
		<div class="fullwidth" id="add_delete_attribute_link_div">
			<div class="title2">
				...
				<span class="title2rightfield-icon1-text">
					<a onclick="addColNames('#varLengthBinaryAttributeList');downloadCSVFile();">
						<i class="fa fa-download"></i>
					</a> 
					<a href="#" onclick="addColNames('#varLengthBinaryAttributeList');downloadCSVFile();">
						<spring:message code="parserConfiguration.attribute.grid.exportattribute.label" ></spring:message>
					</a>
				</span> 
				<sec:authorize access='hasAuthority(\'CREATE_PARSER\')'>
				<span class="title2rightfield-icon1-text"> 
					<a href="#" onclick="openUploadAttrPopup('#varLengthBinaryAttributeList');">
		...
		<div class="col-md-12 inline-form no-padding" id="attribute_grid_div">
			<div class="box-body table-responsive no-padding box"
				id="parser_attribute">
				<table class="table table-hover" id="varLengthBinaryAttributeList"></table>
				<div id="varLengthBinaryAttributeListDiv"></div>
		...
		<form:form modelAttribute="parser_attribute_form_bean" method="POST"
			action="<%=ControllerConstants.ADD_EDIT_VAR_LENGTH_BINARY_PARSER_ATTRIBUTE%>"
			...
			<div class="modal-content">
				...
				<sec:authorize access="hasAuthority('DELETE_PARSER')">
					<div id="delete_attribute_bts_div" class="modal-footer padding10">
						<button id="ascii_del_btn" type="button" class="btn btn-grey btn-xs"
	...
	<form:form modelAttribute="<%=FormBeanConstants.VAR_LENGTH_BINARY_PARSER_MAPPING_FORM_BEAN %>" method="POST" action="" id="var-length-binary-parser-configuration-form">
...
<script type="text/javascript">
	$(document).ready(function() {
			...
			if('${REQUEST_ACTION_TYPE}' == 'VAR_LENGTH_BINARY_HEADER_PARSER_ATTRIBUTE'){
				$("#attribute_basic_details_div").hide();	
			}
			createVarLengthBinaryAttributeGrid('<%=MapCache.getConfigValueAsInteger(SystemParametersConstant.TOTAL_ROWS_TO_DISPLAY_IN_GRID, 10)%>');
			getAllVarLengthBinaryAttributeListByMappingId(urlAction, selMappingId);
			var nextActionType = '';
			if(attributeListType == '<%= BaseConstants.VAR_LENGTH_BINARY_PARSER_ATTRIBUTE %>'){
				nextActionType = '<%= BaseConstants.VAR_LENGTH_BINARY_HEADER_PARSER_ATTRIBUTE %>';
			} 	
...
function addBasicDetail(){
    ...
    $.ajax({
		url: '<%=ControllerConstants.UPDATE_VAR_LENGTH_BINARY_PARSER_MAPPING%>',
		data : oMyForm ,
        type : "POST",
        datatype: "json",
        processData: false, 
...
function getAllVarLengthBinaryAttributeListByMappingId(urlAction, mappingId){
	...
	$.ajax({
		...
		success: function(data){						
			...
			var responseObject = response.object;			
			if(responseCode == "200"){				
				$("#varLengthBinaryAttributeListDiv").show();				
				...
				if(attributeList!=null && attributeList != 'undefined' && attributeList != undefined){					
					...
					$.each(attributeList,function(index,attribute){		
		 				...
		 				rowData.sourceFieldName		= attribute.sourceFieldName;
		 				...
		 				rowData.dateFormatInput     = attribute.dateFormatInput;
		 				rowData.updateAction		= attribute.updateAction;
		 				gridArray.push(rowData);
		 			});
					jQuery("#varLengthBinaryAttributeList").jqGrid('setGridParam', {datatype: 'local', data:gridArray }).trigger("reloadGrid");
				} 
...
function createVarLengthBinaryAttributeGrid(defaultRowNum){	
	 $("#varLengthBinaryAttributeList").jqGrid({	    	
	        datatype: "local",
	        colNames:[
	                  "#",
			...
			colModel:[
			          ...
			          {name: 'checkbox', index: 'checkbox',sortable:false, formatter:varLengthBinaryParserAttributeCheckboxFormatter, align : 'center', width:'30%', search : false},
					  {name: 'unifiedField',index: 'unifiedField',sortable:true},
					  {name: 'description',index: 'description',sortable:false,search : false},
					  {name: 'sourceFieldName',index: 'sourceFieldName',sortable:false,search : false},
					  {name: 'startLength',index: 'startLength',hidden:true,sortable:false,search : false},
					  {name: 'endLength',index: 'endLength',hidden:true,sortable:false,search : false},
					  {name: 'prefix',index: 'prefix',hidden:true,sortable:false,search : false},
					  {name: 'postfix',index: 'postfix',hidden:true,sortable:false,search : false},
					  {name: 'length',index: 'length',hidden:true,sortable:false,search : false},
					  {name: 'rightDelimiter',index: 'rightDelimiter',hidden:true,sortable:false,search : false},
					  {name: 'defaultText',index: 'defaultText',hidden:true,sortable:false,search : false},
					  {name: 'trimChar',index: 'trimChar',hidden:true, sortable:false,search : false},
					  {name: 'trimPosition',index: 'trimPosition',hidden:true, sortable:false,search : false},
					  {name: 'sourceFieldFormat',index: 'sourceFieldFormat', sortable:false,search : false},
					  {name: 'dateFormatInput',index: 'dateFormatInput', sortable:false,search : false},
	        ...
	        height: 'auto',
	        pager: '#varLengthBinaryAttributeListDiv',
	        viewrecords: true,
	        search:true,
	        ignoreCase: true,
	        timeout : 120000,
	 	...
	 	}).navGrid("#varLengthBinaryAttributeListDiv",{edit:false,add:false,del:false,search:true});
		$(".ui-jqgrid-titlebar").hide();
		$('#varLengthBinaryAttributeList').jqGrid('filterToolbar',{
			stringResult: true,
		...
		$("#varLengthBinaryAttributeList").sortableRows();   
		$("#varLengthBinaryAttributeList").disableSelection();
	    $("#varLengthBinaryAttributeList").sortable({
	    	items: 'tr:not(:first)'
	    });
	    
	    $('#varLengthBinaryAttributeList').navButtonAdd('#varLengthBinaryAttributeListDiv', {
           title: "Update Order",
 ...
 */
function varLengthBinaryParserAttributeCheckboxFormatter(cellvalue, options, rowObject) {
	var uniqueId = getUniqueCheckboxIdForAttribute(rowObject);
	return '<input type="checkbox" id="'+uniqueId+'" name="attributeCheckbox" value="'+rowObject["id"]+'" onchange="updateRootCheckbox(this, \'varLengthBinaryAttributeList\')"/>';
}

function updateOrder(){
	console.log("updateOrder Called");
	var ids = $('#varLengthBinaryAttributeList').jqGrid('getDataIDs');
	window.scrollTo(0, 0);
	resetWarningDisplay();
	clearAllMessages();
...
function reloadAsciiAttributeGridData(){
	...
	var $grid = $("#varLengthBinaryAttributeList");
