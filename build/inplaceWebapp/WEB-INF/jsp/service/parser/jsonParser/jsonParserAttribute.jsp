...
<div class="tab-content no-padding  mtop15">
	<div class="fullwidth mtop15" id="attribute_details_div">
		<form:form modelAttribute="<%=FormBeanConstants.JSON_PARSER_MAPPING_FORM_BEAN %>" method="POST"
			id="json_attribute_configuration_form_bean">
		...
		<div class="fullwidth" id="add_delete_attribute_link_div">
			<div class="title2">
				...
				<sec:authorize access='hasAuthority(\'CREATE_PARSER\')'>
				<span class="title2rightfield-icon1-text">
				     	<a href="#" onclick="displayGenerateAttributesPopup();">
						<i class="fa fa-plus-circle"></i>
					</a> 
					<a href="#" onclick="displayGenerateAttributesPopup();"> 
						<spring:message code="parserConfiguration.attribute.grid.generateattributes.label" ></spring:message>
					</a> 
				</span> 
				</sec:authorize>
				<span class="title2rightfield-icon1-text">
					<a onclick="addColNames('#jsonAttributeList');downloadCSVFile();">
						<i class="fa fa-download"></i>
					</a> 
					<a href="#" onclick="addColNames('#jsonAttributeList');downloadCSVFile();">
						<spring:message code="parserConfiguration.attribute.grid.exportattribute.label" ></spring:message>
					</a>
				</span> 
				<sec:authorize access='hasAuthority(\'CREATE_PARSER\')'>
				<span class="title2rightfield-icon1-text"> 
					<a href="#" onclick="openUploadAttrPopup('#jsonAttributeList');">
		...
		<div class="col-md-12 inline-form no-padding" id="attribute_grid_div">
			<div class="box-body table-responsive no-padding box"
				id="parser_attribute">
				<table class="table table-hover" id="jsonAttributeList"></table>
				<div id="jsonAttributeListDiv"></div>
			...
			<div class="modal-content">
				...
				<sec:authorize access="hasAuthority('DELETE_PARSER')">
					<div id="delete_attribute_bts_div" class="modal-footer padding10">
						<button id="json_del_btn" type="button" class="btn btn-grey btn-xs"
...
<a href="#divGenerateAttributes" class="fancybox" style="display: none;" id="generate_attributes">#</a>
 		<div id="divGenerateAttributes" style=" width:100%; display: none;" >
 		  <jsp:include page="generateJsonParserAttrPopup.jsp"></jsp:include>
       </div>
<script type="text/javascript">
	$(document)
			.ready(
					function() {
						...
						var dateFormat = $("#dateFormat").find(":selected")
								.val();
						if (dateFormat != 'Other') {
							$("#srcDateFormat").val('');
	...
	createJsonAttributeGrid('<%=MapCache.getConfigValueAsInteger(SystemParametersConstant.TOTAL_ROWS_TO_DISPLAY_IN_GRID, 10)%>');
...
function addBasicDetail(){
    ...
    $.ajax({
		url: '<%=ControllerConstants.UPDATE_JSON_PARSER_MAPPING%>',
		data : oMyForm ,
        type : "POST",
        datatype: "json",
        processData: false, 
...
function getAllJsonAttributeListByMappingId(urlAction, mappingId){
		...
		success: function(data){						
			...
			if(responseCode == "200"){				
				...
				if(attributeList!=null && attributeList != 'undefined' && attributeList != undefined){					
					...
					jQuery("#jsonAttributeList").jqGrid('setGridParam', {datatype: 'local', data:gridArray }).trigger("reloadGrid");
				} 
...
function createJsonAttributeGrid(defaultRowNum){	
	 $("#jsonAttributeList").jqGrid({	    	
	        datatype: "local",
	        colNames:[
	                  "#",
			...
			colModel:[
			          ...
			          {name: 'checkbox', index: 'checkbox',sortable:false, formatter:jsonParserAttributeCheckboxFormatter, align : 'center', width:'30%', search : false},
	 	...
	 	}).navGrid("#jsonAttributeListDiv",{edit:false,add:false,del:false,search:true});
		$(".ui-jqgrid-titlebar").hide();
		$('#jsonAttributeList').jqGrid('filterToolbar',{
			stringResult: true,
		...
		$("#jsonAttributeList").sortableRows();   
		$("#jsonAttributeList").disableSelection();
	    $("#jsonAttributeList").sortable({
	    	items: 'tr:not(:first)'
	    });
	    
	    $('#jsonAttributeList').navButtonAdd('#jsonAttributeListDiv', {
           title: "Update Order",
 ...
 */
function jsonParserAttributeCheckboxFormatter(cellvalue, options, rowObject) {
	var uniqueId = getUniqueCheckboxIdForAttribute(rowObject);
	return '<input type="checkbox" id="'+uniqueId+'" name="attributeCheckbox" value="'+rowObject["id"]+'" onchange="updateRootCheckbox(this, \'jsonAttributeList\')"/>';
}

function updateOrder(){
	console.log("updateOrder Called");
	var ids = $('#jsonAttributeList').jqGrid('getDataIDs');
	window.scrollTo(0, 0);
	resetWarningDisplay();
	clearAllMessages();
...
function reloadJsonAttributeGridData(){
	clearAllMessages();
	
	var $grid = $("#jsonAttributeList");
