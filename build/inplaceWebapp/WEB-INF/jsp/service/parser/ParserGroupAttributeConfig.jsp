...
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
		...
		<!-- Add/Edit Group Attribute from grid  pop-up code start here-->
 		<form:form modelAttribute="group_attribute_form_bean" method="POST"
			action="<%=ControllerConstants.ADD_EDIT_PARSER_GROUP_ATTRIBUTE%>"
			id="add-edit-groupAttribute-form">
			<input type="hidden" value="0" id="id" name="id" />
			<input type="hidden" value="${plugInType}" id="selplugInType" name="selplugInType" />
			<input type="hidden" value="${mappingId}" id="selConfigMappingId" name="selConfigMappingId"/>
			   <div id="divAddGroupAttribute" style="display: none;">
				 <jsp:include page="addEditViewParserGroupAttributePopUp.jsp"></jsp:include> 
			</div>  
		</form:form>
...
$(document).ready(function() {
	...
	urlActionGrpAttr = '<%=ControllerConstants.GET_GROUP_PARSER_ATTRIBUTE_GRID_LIST_BY_MAPPING_ID%>';
...
function deleteGroupAttribute(){
	...
	$.ajax({
		url:'<%=ControllerConstants.DELETE_GROUP_PARSER_ATTRIBTE %>',
		cache: false,
		async: true,
		dataType:'json',
		type:'POST',
...
function displayAddEditGroupAttributePopup(actionType,attributeId){
	...
	if(actionType === 'ADD'){
		...
		getAllAddGroupAttributeByGroupId('<%=ControllerConstants.GET_ADD_PARSER_GROUP_ATTRIBUTE_GRID_LIST_BY_GROUP_ID%>', selMappingId);
		getAllAddAttributeListByGroupId('<%=ControllerConstants.GET_ADD_PARSER_ATTRIBUTE_GRID_LIST_BY_GROUP_ID%>', selMappingId);
		...
		if(attributeId > 0){
			...
			$("#plugInType").val(currentParserType);
			
			getAllUpdateGroupAttributeByGroupId('<%=ControllerConstants.GET_UPDATE_GROUP_PARSER_ATTRIBUTE_GRID_LIST_BY_GROUP_ID%>', selMappingId, attributeId);
			getAllUpdateAttributeListByGroupId('<%=ControllerConstants.GET_UPDATE_PARSER_ATTRIBUTE_GRID_LIST_BY_GROUP_ID%>', selMappingId, attributeId);
		...
		if(attributeId > 0){
			...
			getAllViewGroupAttributeByGroupId('<%=ControllerConstants.GET_VIEW_GROUP_PARSER_ATTRIBUTE_GRID_LIST_BY_GROUP_ID%>', selMappingId, attributeId);
			getAllViewAttributeListByGroupId('<%=ControllerConstants.GET_VIEW_PARSER_ATTRIBUTE_GRID_LIST_BY_GROUP_ID%>', selMappingId, attributeId);
...
function createViewAttributeListGridOnPopUp(defaultRowNum){				
	 ...
	 $("#viewAttributeList").jqGrid({	    	
	        ...
	        colNames:[
                      "#",
                      "<spring:message code='parser.attr.grid.destination.field.name' ></spring:message>", 
			...
			colModel:[
			          {name: 'id', index: 'id',hidden:true, search : false},
			          {name: 'sourceField', index: 'sourceField'},
...
function getUniqueCheckboxIdForAttributeListOnPopUp(rowObject) {
	...
	if(rowObject["sourceField"] && rowObject["sourceField"] !== 'null' && rowObject["sourceField"] !== "") {
		uniqueId += "_" + rowObject["sourceField"].replace(/ /g, "_");
	}
