...
<div class="tab-content no-padding  mtop15">
	<div class="fullwidth mtop15" id="attribute_details_div">
		<form:form modelAttribute="<%=FormBeanConstants.FIXED_LENGTH_ASCII_PARSER_MAPPING_FORM_BEAN%>" method="POST"
			id="fixedLengthASCII_attribute_configuration_form_bean">
		...
		<div class="fullwidth" id="add_delete_attribute_link_div">
			<div class="title2">
				...
				<span class="title2rightfield-icon1-text">
					<a onclick="addColNames('#fixedLengthASCIIAttributeList');downloadCSVFile();">
						<i class="fa fa-download"></i>
					</a> 
					<a href="#" onclick="addColNames('#fixedLengthASCIIAttributeList');downloadCSVFile();">
						<spring:message code="parserConfiguration.attribute.grid.exportattribute.label" ></spring:message>
					</a>
				</span> 
				<sec:authorize access='hasAuthority(\'CREATE_PARSER\')'>
				<span class="title2rightfield-icon1-text"> 
					<a href="#" onclick="openUploadAttrPopup('#fixedLengthASCIIAttributeList');">
		...
		<div class="col-md-12 inline-form no-padding" id="attribute_grid_div">
			<div class="box-body table-responsive no-padding box"
				id="parser_attribute">
				<table class="table table-hover" id="fixedLengthASCIIAttributeList"></table>
				<div id="fixedLengthASCIIAttributeListDiv"></div>
		...
		<form:form modelAttribute="parser_attribute_form_bean" method="POST"
			action="<%=ControllerConstants.ADD_EDIT_FIXED_LENGTH_ASCII_PARSER_ATTRIBUTE%>"
			...
			<div id="divAdditionalInfoAttribute" style="display: none;">
				<jsp:include page="displayAdditionalFixedLengthASCIIInfoPopUp.jsp"></jsp:include>
...
$(document).ready(function() {
	...
	createFixedLengthASCIIAttributeGrid('<%=MapCache.getConfigValueAsInteger(SystemParametersConstant.TOTAL_ROWS_TO_DISPLAY_IN_GRID, 10)%>');
...
function addBasicDetail(){
    ...
    $.ajax({
		url: '<%=ControllerConstants.UPDATE_FIXED_LENGTH_ASCII_PARSER_MAPPING%>',
		data : oMyForm ,
        type : "POST",
        datatype: "json",
        processData: false, 
...
function createFixedLengthASCIIAttributeGrid(defaultRowNum){				
	 
	 $("#fixedLengthASCIIAttributeList").jqGrid({	    	
	        datatype: "local",
	        colNames:[
	                  "#",
	        ...
	        height: 'auto',
	        pager: '#fixedLengthASCIIAttributeListDiv',
	        viewrecords: true,
	        search:true,
	        ignoreCase: true,
	        timeout : 120000,
	 	...
	 	}).navGrid("#fixedLengthASCIIAttributeListDiv",{edit:false,add:false,del:false,search:true});
		$(".ui-jqgrid-titlebar").hide();
		$('#fixedLengthASCIIAttributeList').jqGrid('filterToolbar',{
			stringResult: true,
		...
		$("#fixedLengthASCIIAttributeList").sortableRows();   
		$("#fixedLengthASCIIAttributeList").disableSelection();
	    $("#fixedLengthASCIIAttributeList").sortable({
	    	items: 'tr:not(:first)'
	    });
	    
	    $('#fixedLengthASCIIAttributeList').navButtonAdd('#fixedLengthASCIIAttributeListDiv', {
           title: "Update Order",
...
function updateOrder(){
	...
	var ids = $('#fixedLengthASCIIAttributeList').jqGrid('getDataIDs');
	window.scrollTo(0, 0);
	resetWarningDisplay();
	clearAllMessages();
...
function getAllfixedLengthASCIIAttributeListByMappingId(urlAction, mappingId){
	...
	$.ajax({
		...
		success: function(data){						
			...
			var responseObject = response.object;			
			if(responseCode == "200"){				
				$("#fixedLengthASCIIAttributeListDiv").show();				
				...
				if(attributeList!=null && attributeList != 'undefined' && attributeList != undefined){					
					...
					jQuery("#fixedLengthASCIIAttributeList").jqGrid('setGridParam', {datatype: 'local', data:gridArray }).trigger("reloadGrid");
				} 
...
function fixedLengthAttributeCheckboxFormatter(cellvalue, options, rowObject) {
	...
	return '<input type="checkbox" id="'+uniqueId+'" name="attributeCheckbox" value="'+rowObject["id"]+'" onchange="updateRootCheckbox(this, \'fixedLengthASCIIAttributeList\')"/>';
}
...
function reloadfixedLengthASCIIAttributeGridData(){
	clearAllMessages();
	
	var $grid = $("#fixedLengthASCIIAttributeList");
