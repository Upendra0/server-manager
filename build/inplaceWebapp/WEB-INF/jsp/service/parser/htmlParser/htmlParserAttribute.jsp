...
<div class="tab-content no-padding  mtop15">
	<div class="fullwidth mtop15" id="attribute_details_div">
		<form:form
			...
			method="POST" id="html_attribute_configuration_form_bean">
			...
			<div class="box box-warning" id="attribute_basic_details_div">
				...
				<div class="box-body inline-form ">
					...
					<div class="col-md-6 no-padding">
						...
						<spring:message code="regex.parser.source.date.format"
							var="tooltip" ></spring:message>
						<div class="form-group">
							...
							<div class="input-group">
								<form:select path="dateFormat"
									...
									title="${tooltip }">
									<c:forEach items="${sourceDateFormatEnum}"
										var="sourceDateFormatEnum">
								...
								<form:input path="srcDateFormat"
									cssClass="form-control table-cell input-sm" tabindex="4"
									id="srcDateFormat" data-toggle="tooltip"
							...
							<div id='buttons-div' class='input-group'>
								<sec:authorize access='hasAuthority(\'UPDATE_PARSER\')'>
									<button type='button' class='btn btn-grey btn-xs'
										id='submitbtn' onclick='addBasicDetail();'>
		...
		<div class="fullwidth" id="add_delete_attribute_link_div">
			<div class="title2">
				...
				<span class="title2rightfield-icon1-text">
					<a onclick="addColNames('#htmlAttributeList');downloadCSVFile();">
						<i class="fa fa-download"></i>
					</a> 
					<a href="#" onclick="addColNames('#htmlAttributeList');downloadCSVFile();">
						<spring:message code="parserConfiguration.attribute.grid.exportattribute.label" ></spring:message>
					</a>
				</span> 
				<sec:authorize access='hasAuthority(\'CREATE_PARSER\')'>
				<span class="title2rightfield-icon1-text"> 
					<a href="#" onclick="openUploadAttrPopup('#htmlAttributeList');">
		...
		<div class="col-md-12 inline-form no-padding" id="attribute_grid_div">
			<div class="box-body table-responsive no-padding box"
				id="parser_attribute">
				<table class="table table-hover" id="htmlAttributeList"></table>
				<div id="htmlAttributeListDiv"></div>
...
<script type="text/javascript">
	...
	function addBasicDetail(){
	    ...
	    $.ajax({
			url: '<%=ControllerConstants.UPDATE_HTML_PARSER_MAPPING%>',
			data : oMyForm ,
	        type : "POST",
	        datatype: "json",
	        processData: false, 
	...
	function getAllhtmlAttributeListByMappingId(urlAction, mappingId){
				...
				if(responseCode == "200"){				
					...
					if(attributeList!=null && attributeList != 'undefined' && attributeList != undefined){					
						...
						$.each(attributeList,function(index,attribute){		 				
			 				...
			 				rowData.fieldIdentifier	    = attribute.fieldIdentifier;
			 				rowData.fieldExtractionMethod=attribute.fieldExtractionMethod;
			 				rowData.fieldSectionId		= attribute.fieldSectionId;	
			 				rowData.containsFieldAttribute=attribute.containsFieldAttribute;
			 				rowData.valueSeparator=attribute.valueSeparator;
			 				rowData.valueIndex=attribute.valueIndex;
			 				rowData.updateAction		= attribute.updateAction;
			 				gridArray.push(rowData);
			 			});
						jQuery("#htmlAttributeList").jqGrid('setGridParam', {datatype: 'local', data:gridArray }).trigger("reloadGrid");
					} 
	...
	function createhtmlAttributeGrid(defaultRowNum){	
		 $("#htmlAttributeList").jqGrid({	    	
		        ...
		        colNames:[
		                  ...
		                  jsSpringMsg.fieldidentifier,
		                  jsSpringMsg.fieldextractionmethod,
		                  jsSpringMsg.fieldSectionId,
		                  jsSpringMsg.containsfieldattribute,
		                  jsSpringMsg.valueseparatorattribute,
		                  jsSpringMsg.valueindexattribute,
				...
				colModel:[
						  ...
						  {name: 'fieldIdentifier',index: 'fieldIdentifier', sortable:false,search : false},
						  {name: 'fieldExtractionMethod',index: 'fieldExtractionMethod', sortable:false,search : false},
						  {name: 'fieldSectionId',index: 'fieldSectionId', sortable:false,search : false},
						  {name: 'containsFieldAttribute',index: 'containsFieldAttribute', sortable:false,search : false},
						  {name: 'valueSeparator',index: 'valueSeparator', sortable:false,search : false},
						  {name: 'valueIndex',index: 'valueIndex', sortable:false,search : false},
		 	...
		 	}).navGrid("#htmlAttributeListDiv",{edit:false,add:false,del:false,search:true});
			$(".ui-jqgrid-titlebar").hide();
			$('#htmlAttributeList').jqGrid('filterToolbar',{
				stringResult: true,
			...
			$("#htmlAttributeList").sortableRows();   
			$("#htmlAttributeList").disableSelection();
		    $("#htmlAttributeList").sortable({
		    	items: 'tr:not(:first)'
		    });
		    
		    $('#htmlAttributeList').navButtonAdd('#htmlAttributeListDiv', {
	           title: "Update Order",
	...
	function htmlParserAttributeCheckboxFormatter(cellvalue, options, rowObject) {
		...
		return '<input type="checkbox" id="'
				+ uniqueId
				+ '" name="attributeCheckbox" value="'
				+ rowObject["id"]
				+ '" onchange="updateRootCheckbox(this, \'htmlAttributeList\')"/>';
	}

	function updateOrder() {
		console.log("updateOrder Called");
		var ids = $('#htmlAttributeList').jqGrid('getDataIDs');
		window.scrollTo(0, 0);
		resetWarningDisplay();
		clearAllMessages();
