...
<div class="tab-content no-padding  mtop15">
	<div class="fullwidth mtop15" id="attribute_details_div">
		<form:form modelAttribute="<%=FormBeanConstants.TAP_COMPOSER_MAPPING_FORM_BEAN %>" method="POST"
		...
		<form:form modelAttribute="composer_attribute_form_bean" method="POST"
			action="<%=ControllerConstants.ADD_EDIT_TAP_COMPOSER_ATTRIBUTE%>"
	...
	<form:form modelAttribute="<%=FormBeanConstants.TAP_COMPOSER_MAPPING_FORM_BEAN %>" method="POST" action="NO_ACTION" id="ascii-composer-configuration-form">
...
$(document).ready(function() {
	...
	createTapAttributeGrid('<%=MapCache.getConfigValueAsInteger(SystemParametersConstant.TOTAL_ROWS_TO_DISPLAY_IN_GRID, 10)%>');
...
function addBasicDetail(){
    ...
    $.ajax({
		url: '<%=ControllerConstants.UPDATE_TAP_COMPOSER_MAPPING%>',
		data : oMyForm ,
        type : "POST",
        datatype: "json",
        processData: false, 
...
/*Function will create JQGRID for TAP composer attribute list */
function createTapAttributeGrid(defaultRowNum){				
	 ...
	 $("#asciiAttributeList").jqGrid({	    	
			...
			colModel:[
			          ...
			          {name: 'checkbox', index: 'checkbox',sortable:false, formatter:tapComposerAttributeCheckboxFormatter, align : 'center', width:'30%', search : false},
...
/*Function will load all TAP composer attributes to JQGRID */
 ...
 */
function tapComposerAttributeCheckboxFormatter(cellvalue, options, rowObject) {
