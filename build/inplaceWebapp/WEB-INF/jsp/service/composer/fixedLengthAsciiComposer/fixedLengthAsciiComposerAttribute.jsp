...
<div class="tab-content no-padding  mtop15">
	<div class="fullwidth mtop15" id="attribute_details_div">
		<form:form modelAttribute="fixed_length_ascii_composer_mapping_form_bean" method="POST"
		...
		<form:form modelAttribute="composer_attribute_form_bean" method="POST"
			action="<%=ControllerConstants.ADD_EDIT_FIXED_LENGTH_ASCII_COMPOSER_ATTRIBUTE%>"
...
function addBasicDetail(){
    ...
    $.ajax({
		url: '<%=ControllerConstants.UPDATE_FIXED_LENGTH_ASCII_BASIC_DETAIL%>',
		data : oMyForm ,
        type : "POST",
        datatype: "json",
        processData: false, 
...
/*Function will create JQGRID for FIXED LENGTH ASCII composer attribute list */
function createFixedLengthASCIIAttributeGrid(defaultRowNum){				
	 ...
	 $("#asciiAttributeList").jqGrid({	    	
	        ...
	        colNames:[
                      ...
                      "<spring:message code='composer.attribute.length'></spring:message>",
			...
			colModel:[
			          ...
			          {name: 'checkbox', index: 'checkbox',sortable:false, formatter:fixedLengthComposerAttributeCheckboxFormatter, align : 'center', width:'30%', search : false},
					  ...
					  {name: 'fixedLengthDateFormat',index: 'fixedLengthDateFormat',sortable:false,hidden:true, search : false},
					  {name: 'fixedLength',index: 'fixedLength',sortable:false,hidden:true, search : false},				 
			...
			if(responseCode == "200"){				
				...
				if(attributeList!=null && attributeList != 'undefined' && attributeList != undefined){					
					...
					$.each(attributeList,function(index,attribute){		 				
		 				...
		 				rowData.fixedLengthDateFormat= attribute.fixedLengthDateFormat;
		 				rowData.trimchars			 = attribute.trimChar;
		 				rowData.fixedLength		     = attribute.fixedLength;
 ...
 */
function fixedLengthComposerAttributeCheckboxFormatter(cellvalue, options, rowObject) {
...
$(document).on("change","#fixedDataType",function(event) {
	diasbleFixedLengthDateFormate();
	
});

function diasbleFixedLengthDateFormate() {
	var dataType = $('#fixedDataType').val();
	if(dataType === 'DATE'){
		$("#fixedLengthDateFormat").attr('readOnly',false);
	}else{
		$("#fixedLengthDateFormat").attr('readOnly',true);
