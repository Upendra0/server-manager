...
var jsSpringMsg = {};
	...
	jsSpringMsg.fixedLengthAsciiComposer='<%=EngineConstants.FIXED_LENGTH_ASCII_COMPOSER_PLUGIN%>';
...
<%--                             		  ${serviceName}&nbsp;-&nbsp;${serviceInstanceId} --%>
                                        ...
                                        <div class="box-body table-responsive no-padding">
                                            <div class="nav-tabs-custom"> 
                                                ...
                                                <ul class="nav nav-tabs pull-right">
                                                    ...
                                                    <li class="<c:if test="${(REQUEST_ACTION_TYPE eq 'FIXED_LENGTH_ASCII_COMPOSER_CONFIGURATION')}"><c:out value="active"></c:out></c:if>" onclick="showButtons('composer_configuration'); getFixedLengthAsciiComposerConfig('<%=ControllerConstants.INIT_FIXED_LENGTH_ASCII_COMPOSER_MANAGER%>');" >
                                                    ...
                                                    <li class="<c:if test="${(REQUEST_ACTION_TYPE eq 'FIXED_LENGTH_ASCII_COMPOSER_ATTRIBUTE')}"><c:out value="active" ></c:out></c:if>" onclick="showButtons('composer_attribute_configuration'); getFixedLengthAsciiComposerAttrDetails('<%=ControllerConstants.INIT_FIXED_LENGTH_ASCII_COMPOSER_ATTRIBUTE%>');" >
                                                ...
                                                <sec:authorize access="hasAnyAuthority('VIEW_COMPOSER')">
                                                	<div class="tab-pane <c:if test="${(REQUEST_ACTION_TYPE eq 'FIXED_LENGTH_ASCII_COMPOSER_CONFIGURATION')}"><c:out value="active"></c:out></c:if>" id="asciiConfiguration">
                                                		 <c:if test="${(REQUEST_ACTION_TYPE eq 'FIXED_LENGTH_ASCII_COMPOSER_CONFIGURATION')}">
                                                			<jsp:include page="fixedLengthAsciiComposerConfiguration.jsp" ></jsp:include>
                                                		</c:if>
                                                	</div>
                                                	
                                                	<div class="tab-pane <c:if test="${(REQUEST_ACTION_TYPE eq 'FIXED_LENGTH_ASCII_COMPOSER_ATTRIBUTE')}"><c:out value="active"></c:out></c:if>" id="asciiAttribute">
                                               			<c:if test="${(REQUEST_ACTION_TYPE eq 'FIXED_LENGTH_ASCII_COMPOSER_ATTRIBUTE')}">
                                               				<jsp:include page="fixedLengthAsciiComposerAttribute.jsp" ></jsp:include>
                                               			</c:if>
                                                	</div>
                                                	</sec:authorize>
        	...
        	<div style="display: block;">
				...
				<form id="fixed-length-ascii-composer-from" action = "" method="">
...
$(document).ready(function() {
	...
	if(composerDeviceTypeId > 0 && currentTab == 'FIXED_LENGTH_ASCII_COMPOSER_CONFIGURATION'){
...
function showHideButtonBasedOnTabsSelected(tabType){
	...
	if(tabType == 'FIXED_LENGTH_ASCII_COMPOSER_CONFIGURATION'){
		$("#asciiComposerConfigDiv").show();
	}else if(tabType == 'ASCII_COMPOSER_ATTR_LIST'){
...
function showButtons(id){
	if(id == 'composer_configuration'){
		showHideButtonBasedOnTabsSelected('FIXED_LENGTH_ASCII_COMPOSER_CONFIGURATION');
	}else if(id == 'composer_attribute_configuration'){
		showHideButtonBasedOnTabsSelected('ASCII_COMPOSER_ATTR_LIST');
	}
}

function getFixedLengthAsciiComposerConfig(action){
	$("#fixed-length-ascii-composer-from").attr("action",action);
	$("#fixed-length-ascii-composer-from").attr("method","GET");
	
	$("#fixed-length-ascii-composer-from #deviceName").val($("#deviceName").find(":selected").text());
	$("#fixed-length-ascii-composer-from #mappingName").val($("#mappingName").find(":selected").text());
	$("#fixed-length-ascii-composer-from #mappingId").val($("#mappingName").find(":selected").val());
	$("#fixed-length-ascii-composer-from #deviceId").val($("#deviceName").find(":selected").val()); 
	
	
	$("#fixed-length-ascii-composer-from").submit();
}
...
function getFixedLengthAsciiComposerAttrDetails(action){
	
	$("#fixed-length-ascii-composer-from").attr("action",action);
	$("#fixed-length-ascii-composer-from").attr("method","POST");
	...
	if($("#deviceName").find(":selected").text()) {
		...
		$("#fixed-length-ascii-composer-from #deviceName").val('${deviceName}');
	}
	if($("#mappingName").find(":selected").text()) {
		$("#fixed-length-ascii-composer-from #mappingName").val($("#mappingName").find(":selected").text());	
	} else {
		$("#fixed-length-ascii-composer-from #mappingName").val('${mappingName}');
	}
	if($("#mappingName").find(":selected").val()) {
		$("#fixed-length-ascii-composer-from #mappingId").val($("#mappingName").find(":selected").val());	
	} else {
		$("#fixed-length-ascii-composer-from #mappingId").val('${mappingId}');
	}
	if($("#deviceName").find(":selected").val()) {
		$("#fixed-length-ascii-composer-from #deviceId").val($("#deviceName").find(":selected").val());	
	} else {
		$("#fixed-length-ascii-composer-from #deviceId").val('${deviceId}');
	}
