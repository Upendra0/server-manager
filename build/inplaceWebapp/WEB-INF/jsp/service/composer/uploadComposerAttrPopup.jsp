...
<!-- MED-8969 START : Attribute upload screen -->
 		  <div class="modal-content">
				...
				<button class="btn btn-grey btn-xs" type="button" id ="upload_parser_attr" 
						onclick="uploadComposerAttrPopup();">
						Upload
					</button>
 ...
 <a href="#divUploadedData" class="fancybox" style="display: none;" id="uploadedData">#</a>
 		<div id="divUploadedData" style=" width:100%; display: none;" >
 			<jsp:include page="uploadedComposerAttrDataSummary.jsp"></jsp:include>
	</div>	
...
function addColNames(gridName){
	...
	var index = colNamesFromGrid.indexOf('Other Details');
	if (index > -1) {
		colNamesFromGrid.splice(index, 1);
	}
	var index2= colNamesFromGrid.indexOf('Edit');
	if( index2 > -1 ){
		colNamesFromGrid.splice(index2, 1);
	}
	var parserType='${plugInType}';
 	if(parserType == 'ASN1_COMPOSER_PLUGIN'){
 		var SeqNo = colNamesFromGrid.indexOf('Seq.No');
 		if (SeqNo > -1) {
 			colNamesFromGrid.splice(SeqNo, 1);
 		}
 		var dateFormat = colNamesFromGrid.indexOf('Date Format');
 		if (dateFormat > -1) {
 			colNamesFromGrid.splice(dateFormat, 1);
 		}
 		var attributeOrder = colNamesFromGrid.indexOf('attributeOrder');
 		if (attributeOrder > -1) {
 			colNamesFromGrid.splice(attributeOrder, 1);
 		}
	}  
...
function uploadComposerAttrPopup() {
	...
	if (!(regex.test(val))) {
		...
		uploadComposerAttrData('<%=ControllerConstants.UPLOAD_COMPOSER_ATTR_DATA%>');
	}
 }
$(document).on("change", "#dataFile", function(event) {
 ...
 function uploadComposerAttrData(urlAction){
