...
<div class="tab-content no-padding  mtop15">
	<div class="fullwidth mtop15" id="attribute_details_div">
		<form:form modelAttribute="xml_composer_mapping_form_bean" method="POST"
			id="xml_attribute_configuration_form_bean">
			...
			<div class="box box-warning" id="attribute_basic_details_div">
				...
				<div class="col-md-6 no-padding">
					...
					<spring:message code="xml.composer.destionation.date.format" var="label"></spring:message>
					<spring:message code="xml.composer.destionation.date.format.tooltip" var="tooltip"></spring:message>
						<div class="form-group">
					...
					<div class="col-md-6 no-padding">
						<spring:message code="xml.composer.destionation.charset" var="label" ></spring:message>
						<spring:message code="xml.composer.destionation.charset.tooltip" var="tooltip" ></spring:message>
						<div class="form-group">
					...
					<div class="col-md-6 no-padding">
						<spring:message code="xml.composer.destination.file.extension" var="label" ></spring:message>
						<spring:message code="xml.composer.destination.file.extension.tooltip" var="tooltip" ></spring:message>
						<div class="form-group">
					...
					<div class='col-md-12 no-padding'> 
							<div class='form-group'> 
								<div id='buttons-div' class='input-group'> 
									...
									<button type='button' class='btn btn-grey btn-xs' id='resetbtn' onclick="resetComposerAttributeParams();"><spring:message code='btn.label.reset'></spring:message></button>&nbsp;  
								</div> 
		...
		<div class="fullwidth" id="add_delete_attribute_link_div">
			<div class="title2">
				...
				<span class="title2rightfield-icon1-text">
					<a onclick="addColNames('#xmlAttributeList');downloadCSVFile();">
							<i class="fa fa-download"></i>
					</a> 
					<a href="#" onclick="addColNames('#xmlAttributeList');downloadCSVFile();">
							<spring:message code="parserConfiguration.attribute.grid.exportattribute.label" ></spring:message>
					</a>
				</span> 				
				<sec:authorize access='hasAuthority(\'CREATE_COMPOSER\')'>
				 <span class="title2rightfield-icon1-text"> 
					<a href="#" onclick="openUploadAttrPopup('#xmlAttributeList');">
		...
		<div class="col-md-12 inline-form no-padding" id="attribute_grid_div">
			<div class="box-body table-responsive no-padding box"
				id="parser_attribute">
				<table class="table table-hover" id="xmlAttributeList"></table>
				<div id="xmlAttributeListDiv"></div>
		...
		<form:form modelAttribute="composer_attribute_form_bean" method="POST"
			action="<%=ControllerConstants.ADD_EDIT_XML_COMPOSER_ATTRIBUTE%>"
...
$(document).ready(function() {
	...
	createXMLAttributeGrid('<%=MapCache.getConfigValueAsInteger(SystemParametersConstant.TOTAL_ROWS_TO_DISPLAY_IN_GRID, 10)%>');
...
function addBasicDetail(){
    ...
    $.ajax({
		url: '<%=ControllerConstants.UPDATE_XML_COMPOSER_MAPPING%>',
		data : oMyForm ,
        type : "POST",
        datatype: "json",
        processData: false, 
...
function xmlParserAttributeCheckboxFormatter(cellvalue, options, rowObject) {
	var uniqueId = getUniqueCheckboxIdForAttribute(rowObject);
	return '<input type="checkbox" id="'+uniqueId+'" name="attributeCheckbox" value="'+rowObject["id"]+'" onchange="updateRootCheckbox(this, \'xmlAttributeList\')"/>';
}


function createXMLAttributeGrid(defaultRowNum){				
	 
	 $("#xmlAttributeList").jqGrid({	    	
	        datatype: "local",
	        colNames:[
	                  "#",
			...
			colModel:[
			          ...
			          {name: 'checkbox', index: 'checkbox',sortable:false, formatter: xmlParserAttributeCheckboxFormatter, align : 'center', width:'30%', search : false},
			          {name: 'sequenceNumber', index: 'sequenceNumber',sortable:true,sorttype: 'number',search:false},
			          {name: 'destinationField', index: 'destinationField',sortable:true,search:true},
					  {name: 'unifiedField',index: 'unifiedField',sortable:true,search : true},
	 	...
	 	}).navGrid("#xmlAttributeListDiv",{edit:false,add:false,del:false,search:true});
		$(".ui-jqgrid-titlebar").hide();
		$('#xmlAttributeList').jqGrid('filterToolbar',{
			stringResult: true,
...
function getAllXMLAttributeListByMappingId(urlAction, mappingId){
		...
		success: function(data){	
			...
			if(responseCode == "200"){				
				...
				if(attributeList!=null && attributeList != 'undefined' && attributeList != undefined){					
					...
					jQuery("#xmlAttributeList").jqGrid('setGridParam', {datatype: 'local', data:gridArray }).trigger("reloadGrid");
				} 
 ...
 */
function xmlComposerAttributeCheckboxFormatter(cellvalue, options, rowObject) {
	 var uniqueId = "checkbox";
	 if(rowObject["sequenceNumber"] && rowObject["sequenceNumber"] !== 'null' && rowObject["sequenceNumber"] !== "") {
		uniqueId += "_" + rowObject["sequenceNumber"];
	 }
...
function xmlComposerUpdateFormatter(cellvalue, options, rowObject){
